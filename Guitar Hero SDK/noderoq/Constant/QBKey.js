// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
// Q B K E Y   G E N E R A T O R
// Big help from Your Local Drug Dealer for this
//
// String goes in, key comes out
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
function GetQBKey(txt, reversed = true)
{
    if (global.Checksums)
    {
        var val = Checksums.Make(txt, reversed);
        
        var rev = Buffer.alloc(4);
        rev.writeUInt32BE(val, 0);
	...
	// Reversed QBKey array
	if (reversed)
	{
		var rev = Buffer.alloc(hex.length);
		
		for (var l=0; l<hex.length; l++)
			rev[rev.length-(l+1)] = hex[l];
