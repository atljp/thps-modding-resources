...
class NodeROQClass
{
	...
	// - - - - - - - - - - - - - - - - - - 
	CompileCommand(inPath, outPath, decompile = false)
	{
		fs.readFile(inPath, (err, data) => {
			if (err)
				return console.log(err);
				
			this.Compile(data, {decompile: decompile}, output => {
				if (!output.result)
					return;
...
if (process.argv.length > 2)
{
	...
	switch (args[0])
	{
        ...
        // Old-school decompile, ROQ context
		case '-d':
			args.shift();
			...
			roq.CompileCommand(inFile, outFile, true);
		break;
	}
}
