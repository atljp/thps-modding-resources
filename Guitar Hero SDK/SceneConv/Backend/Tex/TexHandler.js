...
class TexHandler
{
	constructor() {this.jimp = jimp; this.verbose = false;}
	...
	CreateImgHeader_THAW(data, opt = {})
	{
		...
		if (imgFormat == 'auto')
		{
			var magic = imgFormat.slice(0, 3).toString();
			if (magic == 'DDS')
				imgFormat = 'dds';
			else
				imgFormat = 'png';
		...
		// 32 if DXT, 0 if uncompressed
		w.UInt8(compression == 0 ? 0 : 32);
	...
	async IMGtoDDS(data)
	{
		...
		// THAW images do not have a header!
