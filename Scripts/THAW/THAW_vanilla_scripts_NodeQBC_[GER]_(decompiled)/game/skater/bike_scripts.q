...
script toggle_bike_on_off2 
    ...
    if NOT IsSkaterOnBike
        DoScreenElementMorph id = the_boardstance_sprite Scale = 0
    else
        DoScreenElementMorph id = the_boardstance_sprite Scale = (1.100000023841858, 1.2999999523162842)
    endif
    orient_vehicle
    Skater_ClothAnimOff
    change bike_pedaling = 0
    if NOT IsSkaterOnBike
        if NOT IsBoardMissing
            obj_setboundingsphere 10000
            if GotParam was_not_walking
                Skater_Playanim anim = BrakeToStand Speed = 1.300
                Skater_WaitAnim percent = 50
            endif
            Skater_Playanim anim = THPS7_BoardCatch_Stand backwards Speed = 1.200
            Skater_WaitAnimFinished
            obj_restoreboundingsphere
            switchoffatomic board
            switchoffatomic ped_board
        endif
        Skater_Playanim anim = WStand Cycle BlendPeriod = 0 no_restart
        change load_bike = 1
        SpawnScriptLater swap_paks_for_bike params = {
            heap = unloadableanim
            a = 'unloadable_anims'
            b = 'bmx_anims'
        }
        WaitForEvent type = swap_paks_for_bike_finished
        if GotParam bike_ped_id
            skater_walk_to_bike id = <bike_ped_id>
        endif
        bmx_finish_on
    else
        Skater_Playanim anim = BMX_Getup backwards BlendPeriod = 0 no_restart
        Skater_WaitAnimFinished
        TurnOffSpecialItem
        create_bike_spot
        Skater_Playanim anim = WStand Cycle BlendPeriod = 0 no_restart
