...
replay_current_name = 'Standard'

script PlayAnimSave name = 'Standard' exitscript = ReplayExit_Die CloneFrom = skater
...
script RecordSkaterAnim \{ name = 'Standard' time = 10 }
...
script replay_create_actor 
    ...
    if NOT CompositeObjectExists name = <CloneFrom>
        scriptassert 'Kein Objekt zum Klonen gefunden.'
    endif
...
script record_stop 
    ...
    structure_control_menu_create {
        ...
        control_items = [
            ...
            {text = 'Ende' param = end_time min = 0.0 max = <duration> step = 0.01000}
        ]
        choose_items = [
            {text = 'Exportieren, speichern und ansehen' pad_choose_script = animsave_replay_export pad_choose_params = { id = <ObjID> }}
