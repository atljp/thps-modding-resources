...
script create_stats_menu 
    ...
    if ((GameModeEquals is_singlesession)|| (GameModeEquals is_classic))
        ...
        <combo_text> = ('BESTE KOMBO:' + <combo_string>)
        CreateScreenElement {
            type = TextElement
            parent = sub_menu
            id = stat_combo
...
script stat_poster_add_item 
    ...
    if (<status> = locked)
        ...
        FormatText textname = text 'Lerne \'%a\' zum Freischalten' a = <ability_text>
        rgba = [ 80 80 80 100 ]
    else
        if (<value> = 1)
            string = <alt_text>
...
script update_stats_menu_count 
    ...
    if GameModeEquals is_classic
        ...
        if GotParam overridden
            ...
            FormatText textname = text 'Verfügbare Punkte: %i' i = <stat_value>
        endif
    ...
    SetScreenElementProps id = stat_count text = <text>
    if GotParam overridden
        text = 'DIESE WERTE WERDEN MOMENTAN ÜBERSCHRIEBEN!'
    else
        if sub_menu ::getsingletag can_edit
            get_stats_menu_goal_count name = <name>
            left = (<count> - <stat_value>)
            <row_id> ::getsingletag display_string
            FormatText textname = text '%i %a Punkte verfügbar' i = <left> a = <display_string>
        else
            FormatText textname = text 'Du kannst die Stats deiner Teampartner nicht ändern!' i = <left> a = <display_string>
        endif
    endif
...
script stats_goal_menu_item_add 
    ...
    switch <status>
        ...
        focusable = not_focusable
        default 
        scriptassert 'Unbekannter Zielstatus'
    endswitch
    CreateScreenElement {
        type = ContainerElement
        parent = <parent>
...
script stats_menu_update_row highlight_rgba = [ 100 100 100 80 ] locked_highlight_rgba = [ 50 50 50 80 ] slider_rgba = [ 128 128 128 0 ]
    if IsTrue gary_debug
        if GotParam skater_name
            script_assert 'Skatername loswerden'
        endif
    endif
    if ScreenElementExists id = sub_menu
...
script stats_selection_ped_setup 
    ...
    setup_ped_speech {
        ...
        accept_text = 'Drück \ms um dir die Stat-Challenges anzusehen'
...
script stats_selection_menu_refresh parent = stat_goal_selection_menu last_index = 0
    ...
    if NOT (<alpha> = 0)
        ...
        if (<index> = <max_index>)
            SetScreenElementProps id = tomorrow_text text = 'Keine weiteren Challenges\nbei diesem Shop'
            break 
        endif
        index = (<index> + 1)
        repeat 
