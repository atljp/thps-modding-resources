balance_meter_info = {
    bar_positions = [ (320.0, 165.0) (250.0, 224.0) ]
    bar_positions_mp_h = [ (320.0, 70.0) (250.0, 130.0) ]
    bar_positions_mp_v = [ (140.0, 130.0) (80.0, 224.0) ]
    arrow_positions = [
        (0.0, -17.0)
        (10.0, -17.0)
        (20.0, -15.0)
...
script create_panel_stuff 
    ...
    if InSplitScreenGame
        ...
        CreateScreenElement {
            ...
            texture = special
            Scale = (1.7300000190734863, 1.399999976158142)
            Pos = (10.0, 30.0)
            rot_angle = -0.5000
            just = [ left top ]
            rgba = <special_color>
...
script maybe_show_special_meter 
    if ObjectExists id = skater
        if skater ::IsLocalSkater
            if skater ::AbilityEnabled special
                score_texture = score_small
            else
                score_texture = score_solo
            endif
            SetScreenElementProps id = the_score_sprite texture = <score_texture>
            SetScreenElementLock Off id = the_score_sprite
            if GameModeEquals is_career
                if NOT skater ::AbilityEnabled special
                    if ScreenElementExists id = the_special_bar_special
                        DestroyScreenElement id = the_special_bar_special
                    endif
                    return 
                endif
            endif
            if NOT ScreenElementExists id = the_special_bar_special
                CreateScreenElement {
...
script pause_balance_meter 
    ...
    if ScreenElementExists \{ id = bike_manual_meter }
        SetScreenElementProps \{id = bike_manual_meter tags = { tag_turned_on = 0 }}
        DoScreenElementMorph \{ id = bike_manual_meter alpha = 0 }
    endif
endscript
...
script update_balance_meter_colors 
    ...
    if ((GetGlobalFlag flag = NO_DISPLAY_BALANCE)|| (inside_pause = 1))
        return 
    endif
    ok_color = [ 0 128 0 128 ]
    bad_color = [ 128 0 0 128 ]
    if IsSkaterOnBike
        if Skater_AnimEquals BMX_BackPedal
            SetScreenElementProps id = {<id> child = 0}texture = bike_arrow
            SetScreenElementProps id = {<id> child = 1}rgba = [ 0 0 0 0 ]
            SetScreenElementProps id = {<id> child = 2}rgba = [ 0 0 0 0 ]
            SetScreenElementProps id = {<id> child = 3}rgba = [ 50 50 50 100 ]
            SetScreenElementProps id = <id> Pos = (320.0, 250.0)
            return 
        endif
    endif
    if (<left> = 1)
    ...
    DoScreenElementMorph id = {<id> child = 1}time = 0 alpha = <alpha1> rgba = <color1>
    DoScreenElementMorph id = {<id> child = 2}time = 0 alpha = <alpha2> rgba = <color2>
endscript
...
script Unpause_Balance_Meter 
    ...
    if ScreenElementExists \{ id = bike_manual_meter }
        SetScreenElementProps \{id = bike_manual_meter tags = { tag_turned_on = 1 }}
        DoScreenElementMorph \{ id = bike_manual_meter alpha = 1 }
    endif
endscript
    ...
    if NOT GetGlobalFlag flag = NO_DISPLAY_BALANCE
        if NOT ((InNetGame)& (GetGlobalFlag flag = no_g_display_balance))
            ...
            if IntegerEquals a = <tag_turned_on> b = 1
                ...
                SetScreenElementProps id = {<id> child = 0}rgba = <balance_arrow_color> texture = balancearrow_glow
                SetScreenElementProps id = {<id> child = 3}rgba = <balance_meter_color>
            endif
        endif
    endif
endscript
...
script Create3dArrowPointer Model = 'HUD_arrow/HUD_Arrow.mdl' anglex = 42.50 Pos = (320.0, 50.0) Scale = 0.03000 active_viewport = 0
    if ObjectExists id = <id>
