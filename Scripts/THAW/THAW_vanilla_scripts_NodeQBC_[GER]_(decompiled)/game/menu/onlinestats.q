...
script download_all_stats_files 
    ...
    create_dialog_box {Title = net_status_msg
        text = 'Lade Stats.'
        buttons = [{text = 'Zurück' pad_choose_script = cancel_download_stats}
        ]
        no_animate
    }
    wait 5 frames
...
script StatsRetrievalFailed 
    ...
    dialog_box_exit
    create_dialog_box \{Title = 'Fehlgeschlagen!' text = 'Stats konnten nicht abgerufen werden. Versuch es später noch einmal oder überprüfe deine Netzwerk-Kabel.' buttons = [{ text = 'OK' pad_choose_script = online_stats_exit }]}
endscript
...
script create_online_stats_menu 
    ...
    build_theme_sub_title Title = 'ONLINE-STATS'
...
script online_stats_hmenu_focus 
    ...
    if (<time> = all)
        if NOT (<string> = 'GESAMT')
            RunScriptOnScreenElement online_stats_swap_time_text id = timetext params = { text = 'Gesamt' }
        endif
    endif
    if (<time> = week)
        if NOT (<string> = 'AKTUELL')
            RunScriptOnScreenElement online_stats_swap_time_text id = timetext params = { text = 'Aktuell' }
        endif
    endif
    KillSpawnedScript name = create_online_stat_scores_menu
    KillSpawnedScript name = create_online_stat_players_menu
        ...
        if NOT IsXenon
            if NOT NetSessionFunc obj = presence func = profile_logged_in
                if (<warned> = no)
                    ...
                    create_dialog_box {Title = 'Keine Stats'
                        text = 'Du bist nicht bei GameSpy eingeloggt.\n Du musst bei GameSpy eingeloggt sein, um Stats speichern zu können.'
...
stats_level_names = [
    ...
    { text = 'DOWNTOWN' level = Load_Z_DT }
    { text = 'OST-L.A.' level = Load_Z_EL }
    { text = 'SANTA MONICA' level = Load_Z_SM }
    { text = 'BOHRINSEL' level = Load_Z_OI }
    { text = 'KASINO' level = Load_Z_LV }
    { text = 'SKATE-RANCH' level = Load_Z_SR }
    { text = 'ATLANTA' level = Load_Z_AT }
    { text = 'CHICAGO' level = Load_Z_CH }
    { text = 'MINNEAPOLIS' level = Load_Z_DN }
    { text = 'KYOTO' level = Load_Z_JA }
    { text = 'DIE MALL' level = Load_Z_MA }
    { text = 'MARSEILLES' level = Load_Z_MS }
    { text = 'SANTA CRUZ' level = Load_Z_SZ }
    { text = 'VANS PARK' level = Load_Z_SV }
    { text = 'DIE RUINEN' level = Load_Z_SV2 }
]
