...
script SkaterBloodOn num = 10
    if NOT GotParam name
        script_assert 'Parametr bez nazwy'
    endif
    ...
    if GetTerrainTypeParam param = treadActions terrain_id = <terrain>
        ...
        if GetNextArrayElement (<treadActions>.SkaterSkatingParticleParms)
            ...
            if NOT ObjectExists id = <name>
                ...
                if (<type> = flexible)
                    alloc_flexible_particle params_script = <params_script> name = <name> ObjID = <ObjID> groupid = <groupid>
                else
                    alloc_fast_particle params_script = <params_script> name = <name> attachobjid = <ObjID> groupid = <groupid>
                endif
            endif
        else
            break 
...
script SkaterParticles_CreateStepGfx bone = Bone_Ankle_L systemLifetime = 400
    ...
    if GetTerrainTypeParam param = treadActions terrain_id = <terrain>
        ...
        if GetNextArrayElement (<treadActions>.SkaterStepParticleParms)
            ...
            params_script = {(<element>)params = <new_params>}
            if (<type> = flexible)
                alloc_flexible_particle params_script = <params_script> name = <name> ObjID = <ObjID> groupid = <groupid> bone = <bone>
            else
                if StructureContains structure = (<element>)emitduration
                    systemLifetime = (1000 * (<element>.emitduration))
                    CastToInteger systemLifetime
                endif
                alloc_fast_particle params_script = <params_script> name = <name> groupid = <groupid> bone = <bone> systemLifetime = <systemLifetime>
...
script skater_sparks_emit 
    ...
    setparticlesize sw = 2 sh = 5 ew = 0.5000 eh = 1.250
...
skater_elec_sparks_params = {
    ...
    LocalSpace
    add_vel
    params = { update_pos = 1 get_bone_script = GetBoardGrindBone }
    max = 40
...
script skater_elec_sparks_emit 
    ...
    setparticlesize sw = 2.500 sh = 6.250 ew = 1.500 eh = 3.750
