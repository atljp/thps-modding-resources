goal_goalattack_genericParams = {
    goal_text = 'Musisz osi¹gn¹æ wszystkie cele ¿eby wygraæ!'
    unlimited_time = 1
    no_restart
    net
...
script goal_goalattack_expire 
    ...
    if OnServer
        ...
        SpawnScriptLater \{wait_then_create_rankings params = { score_title_text = 'CELE' }}
    else
        create_rankings \{ score_title_text = 'CELE' }
    endif
endscript
...
script create_choose_goals_menu 
    ...
    build_theme_sub_title Title = 'WYBIERZ ZADANIA'
    CreateScreenElement {
        type = ContainerElement
        parent = goals_anchor
        id = goals_menu
...
script select_all_no_goals_menu_set_event 
    ...
    CreateScreenElement {
        ...
        just = [ left center ]
        font = small
        text = 'Wybierz wszystkie/¿adne zadania'
        Scale = 0.8000
        rgba = <off_rgba>
    }
    GetScreenElementDims id = <id>
...
script create_view_selected_goals_menu 
    ...
    if GotParam goal_summary
        build_theme_sub_title Title = 'LISTA ZADAÑ'
    else
        build_theme_sub_title Title = 'PRZEJRZYJ ZADANIA'
    endif
    CreateScreenElement {
        type = ContainerElement
        parent = goals_anchor
...
script goal_attack_completed_goal 
    FormatText textname = msg_text 'Zadanie ukoñczone! Pozosta³o: %i' i = <NumGoalsLeft>
    create_net_panel_message text = <msg_text> style = net_team_panel_message
endscript

script goal_attack_completed_goal_other_same_team 
    FormatText textname = msg_text '%n ukoñczonych %t! %i pozosta³o!' n = <playername> t = <GoalText> i = <NumGoalsLeft>
    create_net_panel_message text = <msg_text> style = net_team_panel_message
endscript

script goal_attack_started_goal_other_same_team 
    FormatText textname = msg_text '%n rozpoczyna %t!' n = <playername> t = <GoalText>
    create_net_panel_message text = <msg_text> style = net_team_panel_message
endscript
