...
script ingame_confirm_save \{ no_script = create_pause_menu yes_script = launch_ingame_menu_save_game_sequence }
    create_snazzy_dialog_box {
        Title = 'Zapisz'
        text = 'Czy chcia³by zapisaæ swoje postêpy?'
        text_dims = (400.0, 0.0)
        no_bg
        buttons = [{font = small text = 'Nie' pad_choose_script = <no_script>
            }
            {font = small text = 'Tak' pad_choose_script = <yes_script>
            }
        ]
    }
endscript
...
script game_progress_debug_menu_create 
    ...
    CreateScreenElement {
        ...
        parent = progress_debug_anchor
        font = testtitle
        text = 'Odblokowywanie debugu atomu'
        just = [ left top ]
        Scale = 1.600
        rgba = [ 4 45 120 128 ]
        not_focusable
    ...
    create_helper_text helper_text_elements = [{ text = '\m0 = Odblokuj' }{ text = '\m1 = Wyjd' }]
