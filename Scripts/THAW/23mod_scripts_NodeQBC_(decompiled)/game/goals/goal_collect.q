Goal_Collect_GenericParams = {
    goal_text = 'Zbierz x rzeczy!'
    view_goals_text = 'Ogólna informacja o zebraniu'
    goal_description = 'Standardowy tekst zadania typu \'zbierz\''
    time = 120
...
script AddGoal_Collect 
    ...
    if NOT GotParam goal_collect_objects
        script_assert 'AddGoal_Collect - Cel zbierania stworzony bez goal_collect_objects.'
    endif
    if GotParam version
...
script goal_collect_got_object 
    ...
    if NOT GoalManager_AllFlagsSet name = <goal_id>
        FormatText textname = collect_text '%c z %n %t' c = <num_flags_set> t = <collect_type> n = <num_flags>
