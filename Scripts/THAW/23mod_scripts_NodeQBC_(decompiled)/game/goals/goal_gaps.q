Goal_Gaps_genericParams = {
    goal_text = 'ogólny tekst luki'
    view_goals_text = 'Zadanie luk'
    time = 120
    init = goal_gaps_init
    uninit = goal_uninit
...
script goal_mini_combo_land_prompt 
    create_panel_block \{ text = 'WIETNIE! Teraz zakoñcz!' style = panel_message_landed id = goal_mini_combo_land_msg }
endscript
...
script goal_gaps_got_gap 
    ...
    FormatText textname = got_gap_text '%c z %n %s' c = <num_flags_set> n = <num_flags> s = <collect_type>
...
script goal_gaps_hold_trick_checker trick_time = 10
    ...
    if NOT GotParam trick_type
        scriptassert 'Goal_gaps typu hold_trick zosta³o uruchomione bez wyznaczonego trick_type'
    endif
    ...
    switch <trick_type>
        ...
        default 
        scriptassert 'goal_gaps_hold_trick_checker uzyska³ nieobs³ugiwany trik: %s' s = <trick_type>
    endswitch
    SetException {
        Ex = <exception>
...
script goal_gaps_hold_trick_timer 
    ...
    if (<trick_time_left> < 1)
        KillSpawnedScript name = goal_gaps_hold_trick_check_specific
        if (<land_required> = 1)
            create_panel_block id = goal_gaps_trick_time_msg text = 'WIETNIE! Teraz zakoñcz!' style = panel_message_landed
            SpawnScriptLater {
    ...
    FormatText textname = time_left_text 'Pozosta³o %a sek.' a = <trick_time_left>
...
script score_battle_expire 
    score_battle_cleanup
    create_panel_message \{ id = goal_message text = 'Koniec czasu!' style = panel_message_generic_loss }
...
script score_battle_create_scoreboard name_font = small name_rgba = [ 128 128 128 128 ] opponent_name = 'Inny kole'
...
script goal_contest_next_round 
    create_panel_message \{ id = goal_contest_next_round_msg text = 'Nastêpna runda!' style = panel_message_goalcompleteline2 }
    PlayStream \{ VansTakeLead vol = 160 priority = StreamPriorityHigh }
endscript

script goal_contest_eliminate 
    FormatText textname = text '%s wyeliminowany!' s = <name>
...
script goal_buy_activate 
    ...
    if ((<cash> > <buy_goal_cost>)|| (<cash> = <buy_goal_cost>))
        ...
        <buttons> = [
            {
                text = 'Wrócê jak bêdê mia³ wiêcej pieniêdzy.'
                pad_choose_script = goal_buy_pay_later
            }
        ]
    endif
    FormatText textname = text 'Musisz zap³aciæ: $%a\nPosiadasz: $%b' a = <buy_goal_cost> b = <cash>
    ...
    if NOT skater ::IsInBail
        if NOT SkaterCurrentScorePotGreaterThan 0
            ...
            create_speech_box {
                ...
                text = 'Wcinij \ms, by wykonaæ specjalny transfer'
                no_pad_choose
                no_pad_start
...
script goal_transfer_special_warn_msg 
    GoalManager_GetGoalParams name = <goal_id>
    FormatText textname = message '%s potrzebuje specjala!!!!' s = <full_name>
    create_panel_message {
...
script goal_transfer_special_transfer_msg 
    create_panel_message \{ id = goal_current_reward text = 'Dziêki! Utrzymaj dostawy!' style = goal_message_bouncy_blue_midscreen }
endscript

script goal_transfer_special_no_transfer_msg 
    create_panel_message \{ id = goal_current_reward text = 'Nie masz specjala!' style = goal_message_bouncy_blue_midscreen }
endscript
