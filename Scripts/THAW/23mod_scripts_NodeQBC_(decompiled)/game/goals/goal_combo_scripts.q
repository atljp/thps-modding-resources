Goal_amateurCOMBOline_genericParams = {
    goal_text = 'Zbierz C-O-M-B-O'
    view_goals_text = 'Zbierz C-O-M-B-O'
    goal_flags = [
        got_C
        got_O
        got_M
...
Goal_ProCOMBOline_genericParams = {
    goal_text = 'Zbierz C-O-M-B-O'
    view_goals_text = 'Profesjonalne litery C-O-M-B-O'
    goal_flags = [
        got_C
        got_O
        got_M
...
script ComboLetter_HUDLetter 
    ...
    if GotParam text
        ...
        script_assert 'nie znaleziono tekstu do zadania \'litery combo\''
    endif
endscript
...
script ComboLetters_FailedMessage 
    if NOT GameModeEquals is_coop
        create_panel_message id = new_goal_message text = 'Straci³e swoje combo!' style = panel_message_generic_loss
    else
        MangleChecksums a = 0 b = 0
        if (<skater_id> = <mangled_id>)
            create_panel_message id = new_goal_message text = 'Straci³e swoje combo!' style = panel_message_generic_loss0
        else
            create_panel_message id = new_goal_message text = 'Straci³e swoje combo!' style = panel_message_generic_loss1
        endif
    endif
endscript
