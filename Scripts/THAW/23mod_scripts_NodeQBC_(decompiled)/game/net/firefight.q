goal_firefight_genericParams = {
    goal_text = 'Musisz wyeliminowaæ pozosta³ych graczy!'
    unlimited_time = 1
    net
...
script goal_firefight_activate 
    if InTeamGame
        FormatText textname = msg_text '\c2Strzelanina! \n\c0Ostatni pozosta³y przy ¿yciu gracz, wygrywa.\nAby strzeliæ, wcinij \b1\b7 albo \b1\b4'
    else
        FormatText textname = msg_text '\c2Strzelanina! \n\c0Ostatni ¿ywy gracz wygrywa.\n\b1\b7 lub \b1\b4\nby strzeliæ'
    endif
    if NOT IsObserving
...
script goal_firefight_finished 
    ...
    report_stats \{ final }
    create_rankings \{ score_title_text = 'ZDROWIE' }
endscript
...
script goal_firefight_expire 
    ...
    if OnServer
        SpawnScriptLater \{wait_then_create_rankings params = { score_title_text = 'Zdrowie' }}
    else
        create_rankings \{ score_title_text = 'Zdrowie' }
    endif
endscript
...
script announce_elimination 
    ...
    FormatText textname = msg_text '%s zosta³ wyeliminowany!' s = <name>
