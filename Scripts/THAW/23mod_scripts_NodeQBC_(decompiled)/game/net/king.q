...
goal_king_genericParams = {
    goal_text = 'Zdob¹d koronê i nie pozwól, by inni j¹ dorwali!'
    unlimited_time = 1
    Score = 10000
    net
...
script goal_king_activate 
    ...
    switch <Score>
        case 30000
        time_string = '30 sekund'
        case 60000
        time_string = '1 minuta'
        case 120000
        time_string = '2 minuty'
        case 300000
        time_string = '5 minut'
        case 600000
        time_string = '10 minut'
    endswitch
    if InTeamGame
        FormatText textname = msg_text '\c3Król na wzgórzu:\n\c0Utrzymaj koronê przez \n%s by zwyciê¿yæ.' s = <time_string>
    else
        FormatText textname = msg_text '\c3Król na wzgórzu:\n\c0Utrzymaj koronê przez \n%s by zwyciê¿yæ.' s = <time_string>
    endif
...
script goal_king_finished 
    ...
    report_stats \{ final }
    create_rankings \{ score_title_text = 'CZAS' }
endscript
