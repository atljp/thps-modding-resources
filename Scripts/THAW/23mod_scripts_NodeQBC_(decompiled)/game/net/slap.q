goal_slap_genericParams = {
    goal_text = 'Musisz klepn¹æ najwiêcej graczy, by wygraæ!'
    time = 10
    net
...
script goal_slap_activate 
    if InTeamGame
        FormatText textname = msg_text '\c2Slap! \n\c0Wygrywa dru¿yna, która najwiêcej razy zdzieli przeciwników.'
    else
        FormatText textname = msg_text '\c2Klaps! \n\c0Gracz, który rozda najwiêcej klapsów wygrywa.'
    endif
...
script goal_slap_expire 
    ...
    if OnServer
        SpawnScriptLater \{wait_then_create_rankings params = { score_title_text = 'CIOSY' }}
    else
        create_rankings \{ score_title_text = 'CIOSY' }
    endif
endscript
