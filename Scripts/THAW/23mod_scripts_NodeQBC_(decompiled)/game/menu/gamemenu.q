...
script maybe_edit_skater 
    ...
    wait 2 frames
    create_dialog_box {Title = 'Potwierdzenie edycji'
        text = 'Opuciæ aktualny poziom, by zmieniæ ustawienia?'
        Pos = (320.0, 240.0)
        just = [ center center ]
        text_rgba = [ 88 105 112 128 ]
        text_scale = 1
        pad_back_script = no_edit_skater
        buttons = [{font = small text = 'Tak' pad_choose_script = <yes_script> pad_choose_params = <yes_params>}
            {font = small text = 'Nie' pad_choose_script = no_edit_skater}
        ]
    }
endscript
...
script create_restart_menu 
    ...
    theme_menu_add_item text = 'Zrobione' pad_choose_script = create_debug_options_menu no_bg last_item = last_item centered = centered
...
script menu_confirm_quit {
    ...
    if NOT IsTrue bootstrap_build
        create_error_box {Title = <Title>
            ...
            buttons = [{font = small text = 'Tak' pad_choose_script = <yes_script> pad_choose_params = {level = Load_Z_MainMenu <params>}}
                {font = small text = 'Nie' pad_choose_script = <no_script> pad_choose_params = <params>}
            ]
        }
    else
        create_error_box {Title = <Title>
            ...
            text_rgba = [ 88 105 112 128 ]
            pad_back_script = <back_script>
            buttons = [{font = small text = 'Tak' pad_choose_script = bootstrap_quit}
                {font = small text = 'Nie' pad_choose_script = <back_script>}
            ]
        }
    endif
endscript
...
script menu_confirm_quit_park_editor {
    ...
    if IsParkUnsaved
        if ((IsXbox)|| (IsXenon)|| (isPC))
            error_text = 'Czy chcesz przed wyjciem zapisaæ swój park?'
        else
            if IsNGC
                error_text = 'Czy chcesz przed wyjciem zapisaæ swój park?'
            else
                error_text = 'Gra w sieci, lub ci¹gniêcie nowego parku sprawi, ¿e utracisz wszystkie niezapisane zmiany w tym parku. Zapisaæ go?'
            endif
        endif
        create_error_box {Title = <Title>
            text = <error_text>
            ...
            text_rgba = [ 88 105 112 128 ]
            pad_back_script = <back_script>
            buttons = [{font = small text = 'Tak' pad_choose_script = <yes_script> pad_choose_params = { level = Load_Z_MainMenu }}
                {font = small text = 'Nie' pad_choose_script = <no_script> pad_choose_params = { level = Load_Z_MainMenu }}
            ]
        }
    else
        <no_script> level = Load_Z_MainMenu
...
script create_watch_cutscenes_menu \{ back_script = create_options_menu }
    ...
    make_new_themed_sub_menu \{ Title = 'PRZERYWNIKI' }
    SetScreenElementProps {id = sub_menu
        event_handlers = [
    ...
    theme_menu_add_item text = 'Zrobione' pad_choose_script = generic_menu_pad_choose pad_choose_params = { callback = <back_script> }centered last_menu_item = last_menu_item
...
script create_options_score_display_menu 
    ...
    make_new_themed_sub_menu Title = 'TRYB WYWIETLANIA WYNIKU' right_bracket_alpha = 0.0
    SetScreenElementProps {
        id = current_menu
        event_handlers = [{pad_back generic_menu_pad_back params = { callback = back_from_boolean_menus }}]
        replace_handlers
    }
    theme_menu_add_item text = 'Poka¿ graczy' centered id = menu_players pad_choose_script = choose_boolean_option pad_choose_params = {field = 'score_display' checksum = score_players string = 'Poka¿ graczy'}
    theme_menu_add_item text = 'Poka¿ dru¿yny' centered id = menu_teams pad_choose_script = choose_boolean_option pad_choose_params = {field = 'score_display' checksum = score_teams string = 'Poka¿ dru¿yny'}
    theme_menu_add_item text = 'Zrobione' pad_choose_script = back_from_boolean_menus last_menu_item = last_menu_item
    finish_themed_sub_menu
endscript
...
script create_your_options_menu 
    ...
    if GameModeEquals is_career
        add_label_menu_item {
            text = 'OBEJRZYJ  UMIEJÊTNOCI'
            rot = 1
            Scale = 0.9000
            pad_choose_script = create_view_abilities_menu
        }
        add_label_menu_item {
            text = 'PRZEJRZYJ  ELEMENTY  ORODKA'
            rot = 1
            Scale = 0.9000
...
script create_debug_options_menu 
    ...
    add_label_menu_item {
        ...
        Length = 180
        label_offset = (-20.0, 0.0)
        id = menu_cheats pad_choose_script = create_cheats_menu
    }
    add_label_menu_item {
        text = 'RESTARTS'
        rot = -4
    ...
    add_label_menu_item {
        ...
        Length = 180
        label_offset = (-20.0, 0.0)
        id = #"0x0c73d906" pad_choose_script = create_debug_menu
    }
    add_label_menu_item {
        text = 'TEST MENU'
        rot = -4
        Length = 180
        label_offset = (-20.0, 0.0)
        id = menu_zoneloads pad_choose_script = create_test_menu
    }
