...
script pager_window_build_menu 
    ...
    if NOT InInternetMode
        pager_window_add_menu_item text = 'ODPOWIED KONTEKSTOWA' id = context_reply pad_choose_script = pager_window_context_reply
        pager_window_add_menu_item text = 'POPRZEDNIA WIADOMOÆ' id = prev pad_choose_script = pager_window_prev_message
    endif
    pager_window_add_menu_item text = 'NASTÊPNA WIADOMOÆ' id = next pad_choose_script = pager_window_next_message
    if InInternetMode
        if GotParam nick
            pager_window_add_menu_item text = 'ODPOWIEDZ' id = reply pad_choose_script = message_buddy pad_choose_params = { <...> from_buddy_message }
        endif
    endif
    pager_window_add_menu_item text = 'WYJCIE' id = exit pad_choose_script = pager_window_destroy
...
script pager_window_update_message_title 
    if NOT InInternetMode
        if (<PagerMessageCount> = 0)
            ...
            FormatText textname = Title 'WIADOMOÆ %a:' a = <CurrentMessageNum>
        endif
    endif
...
script pager_window_update_message_pane 
    if InInternetMode
        ...
        if (<PagerMessageCount> = 0)
            ...
            if GotParam from
                FormatText textname = message 'OD <%a>\n\n%b' a = <from> b = <message>
            endif
        endif
    endif
...
script pager_window_refresh_content 
    if InInternetMode
        FormatText textname = Title 'Wiadomoæ od %s' s = <nick>
