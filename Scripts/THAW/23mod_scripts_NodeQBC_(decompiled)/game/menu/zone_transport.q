...
script bus_teleport 
    ...
    if NOT walking
        text = '¯adnego je¿d¿enia w autobusie!\n\n(Wcinij \mm by wróciæ do chodzenia.)'
    else
        if currentscorepotgreaterthan 0
            text = 'Zanim wejdziesz, zakoñcz combo!'
        else
            speech_box_exit
...
script zone_transport_menu_add_item 
    if GotParam prefix
        ...
        if LevelIs <level_checksum>
            printf 'Nie wywietlono %a, gdy¿ jest on w obecnej strefie' a = <prefix>
            return 
        endif
    endif
    if GotParam prefix
        ...
        if GotParam bus_restart_node
            ...
            if NOT IsTrue All_Levels_Unlocked
                if NOT GetFlag level = <dest_check> flag = LEVEL_UNLOCKED_STORY
                    printf 'Poziom %a nie jest odblokowany!' a = <dest>
...
script skateranch_truck_load 
    ...
    if NOT GetGlobalFlag flag = CAREER_SR_TEMP_LOCKED
        create_snazzy_dialog_box {
            Title = 'Gratulacje!'
            text = 'Zdoby³e now¹ czêæ do Orodka!\nWracasz do Orodka?'
            text_dims = (400.0, 0.0)
            pad_back_script = skateranch_truck_cancel
            no_bg
            buttons = [
                {text = 'Tak' pad_choose_script = skateranch_truck_go}
                {text = 'Nie' pad_choose_script = skateranch_truck_cancel}
            ]
        }
    else
        create_snazzy_dialog_box {
            ...
            no_bg
            buttons = [
                {text = 'Ch³odno' pad_choose_script = skateranch_truck_cancel}
            ]
        }
    endif
endscript
...
script skateranch_truck_go 
    ...
    wait \{ 1 seconds }
    printf \{ 'Przeja¿d¿ka do siedziby skejtów' }
...
script skateranch_truck_menu 
    ...
    if NOT walking
        ...
        if currentscorepotgreaterthan 0
            text = 'Zakoñcz combo zanim wsi¹dziesz do ciê¿arówki!'
        else
            speech_box_exit
            speech_box_exit anchor_id = goal_start_dialog
            speech_box_exit anchor_id = ped_speech_dialog
