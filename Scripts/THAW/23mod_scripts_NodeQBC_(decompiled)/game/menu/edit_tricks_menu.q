...
script create_edit_tricks_menu 
    ...
    if (<tricks_editable> = 0)
        <menu_title> = 'PRZEJRZYJ TRIKI'
    else
        <menu_title> = 'EDYTUJ TRIKI'
    endif
    if NOT InFrontend
    ...
    make_label_menu id = edit_tricks_pre_menu Pos = (130.0, 110.0) Title = <menu_title> title_pos = (150.0, 20.0) title_rotation = -3 pad_back_script = generic_menu_pad_back pad_back_params = <menu_back_params>
    add_label_menu_item {
        text = 'Graby'
        rot = 3
...
script create_edit_tricks_sub_menu 
    ...
    build_theme_sub_title Title = <Title> <grad_bar> rot = 2
    ...
    if (<type> = lip_tricks)
        ...
        <tap_info_text> = 'Poj. wciniêcie / Podw. Wciniêcie'
    endif
...
script special_tricks_menu_add_special_to_mapping 
    ...
    if (<special_trickname> = Unassigned)
        <trick_display_text> = 'Nieprzypisane'
    else
        if (<iscat> = 1)
...
script special_tricks_menu_add_trick 
    ...
    if NOT (<type> = CAT || <type> = CATGrind || <type> = CATManual)
...
script special_tricks_menu_select_trick 
    ...
    switch <type>
        ...
        default 
        printstruct <...> 
        script_assert 'Nieznany typ'
    endswitch
    printf 'type = %i' i = <type>
    if (<type> = CAT || <type> = CATGrind || <type> = CATManual)
        printf 'type = cat'
        cat_pad_choose_params = {createdtrick = createdtrick
            cat_num = <cat_num>
        }
    else
        printf 'type doesn\'t = cat'
        cat_pad_choose_params = {}
    endif
    GetArraySize <key_combo_array>
    <key_combo_index> = 0
    begin 
    ...
    edit_tricks_sub_menu_add_key_combo {
        ...
        pad_choose_params = {<cat_pad_choose_params>
            id = <anchor_id>
            highlight_script = edit_tricks_menu_key_combo_focus
            highlight_bar_scale = <short_highlight_bar_scale>
            key_combo = (<key_combo_array> [ <key_combo_index> ])
...
special_trick_types = [
    { type = FlipTrick heading = 'Triki w powietrzu' add_cats }
    { type = GrabTrick heading = 'Graby' add_cats }
    { type = LipMacro2 heading = 'Lip Triki' }
    { type = GrindTrick heading = 'Grindy' add_cats }
    { type = Manual heading = 'Manuale' add_cats }
]
