...
script set_sub_bg \{parent = current_menu_anchor Pos = (326.0, 115.0) Scale = (1.1699999570846558, 1.100000023841858) just = [ center top ]}
    CreateScreenElement {
        type = SpriteElement
        parent = <parent>
        texture = options_bg
...
script hide_all_hud_sprites 
    ...
    hide_stat_message
    hide_comp_text
    hide_score_battle_scoreboard
    goal_hide_all_floaters
    hide_hold_trick_msg
    hide_tips
    pause_trick_text
    hide_death_msg
...
script show_all_hud_sprites 
    ...
    TantrumMessageShow
    unhide_score_battle_scoreboard
    goal_unhide_all_floaters
    unhide_hold_trick_msg
    unpause_trick_text
    if NOT InNetGame
        unpause_trick_text
    endif
...
script change_gamemode_cat 
    printf \{ '********** CHANGING GAME MODE TO CAT' }
    EnableSun
    SetGameType \{ CAT }
    SetCurrentGameType
endscript

script change_gamemode_cap 
    printf \{ '********** CHANGING GAME MODE TO CAP' }
    EnableSun
    SetGameType \{ ParkEditor }
    SetCurrentGameType
endscript
...
script toggle_menu_item_on \{ toggle_text = 'on' }
...
script toggle_menu_item_off \{ toggle_text = 'off' }
    SetScreenElementProps {
        id = {<id> child = 0}
        text = <toggle_text>
    }
...
script animate_in 
    ...
    DoMorph \{ time = 0.05000 rot_angle = 3 Pos = (40.0, 0.0) alpha = 1 }
    DoMorph \{ time = 0.01000 rot_angle = 0 Pos = (10.0, 0.0) }
...
script pause_show_deck active_viewport = 0 node = TRG_Restart01 parent = root_window
    ...
    if GetPlayerAppearancePart player = <ObjID> part = board
        ...
        SpawnScriptLater rotate_pause_deck <no_slide>
    endif
endscript
