...
script game_progress_menu_create 
    ...
    if NOT InFrontend
        ...
        main_menu_camera_control next_state = optprogress
    endif
    ...
    build_theme_sub_title Title = 'POSTÊP W GRZE'
    if InFrontend
    else
    endif
    CreateScreenElement {
    ...
    progress_menu_add_item id1 = progress_vmenu left_col_text = 'MISJE TRYBU FABULARNEGO:' right_col_text = <goal_points>
    GetClassicLevelGoalCount
    FormatText textname = goal_points '%i / %j' i = <classic_total_goals> j = <classic_potential_goals>
    progress_menu_add_item id1 = progress_vmenu left_col_text = 'MISJE TRYBU KLASYCZNEGO:' right_col_text = <goal_points>
    progress_menu_add_item id1 = progress_vmenu seperator
    GetCompletionStatusText mode = GAME_COMPLETED_BEGINNER
    progress_menu_add_item id1 = progress_vmenu left_col_text = 'FABULARNY - £ATWY:' right_col_text = <completion_status>
    GetCompletionStatusText mode = GAME_COMPLETED_NORMAL
    progress_menu_add_item id1 = progress_vmenu left_col_text = 'FABULARNY - NORMALNY:' right_col_text = <completion_status>
    GetCompletionStatusText mode = GAME_COMPLETED_SICK
    progress_menu_add_item id1 = progress_vmenu left_col_text = 'FABULARNY - CHORY:' right_col_text = <completion_status>
    GetCompletionStatusText mode = GAME_COMPLETED_CLASSIC_NORMAL
    progress_menu_add_item id1 = progress_vmenu left_col_text = 'KLASYCZNY - NORMALNY:' right_col_text = <completion_status>
    GetCompletionStatusText mode = GAME_COMPLETED_CLASSIC_SICK
    progress_menu_add_item id1 = progress_vmenu left_col_text = 'KLASYCZNY - CHORY:' right_col_text = <completion_status>
    progress_menu_add_item id1 = progress_vmenu seperator
        ...
        if NOT (((level_select_menu_level_info [ <index> ]).level)= Load_TestLevel)
            if NOT (((level_select_menu_level_info [ <index> ]).level)= load_default)
                if NOT StructureContains structure = ((level_select_menu_level_info [ <index> ]))DEVKIT_ONLY
                    <show_me> = 1
                    if StructureContains structure = ((level_select_menu_level_info [ <index> ]))collectors_edition_only
                        if NOT IsCollectorsEdition
                            <show_me> = 0
                        endif
                    endif
                    if (<show_me>)
                        GetLevelGapTotals level = ((level_select_menu_level_info [ <index> ]).level)
                        if (<num_gaps> = 0)
                            <gaps_text> = '0 / ??'
                        else
                        ...
                        if (All_Levels_Unlocked = 1)
                            ...
                            show_name = 0
                            if StructureContains structure = (level_select_menu_level_info [ <index> ])level
                                if GetFlag flag = LEVEL_VISITED level = ((level_select_menu_level_info [ <index> ]).level)
                                    show_name = 1
                                endif
                            endif
                        endif
                        if (<show_name> = 1)
                            FormatText textname = level_text ' %s LUK:' s = ((level_select_menu_level_info [ <index> ]).text)
                        else
                            FormatText textname = level_text ' %s LUK:' s = '??????'
                        endif
...
script GetCompletionStatusText 
    if (GetGlobalFlag flag = <mode>)
        <completion_status> = 'UKOÑCZONY!'
    else
        <completion_status> = 'NIEUKOÑCZONY'
    endif
