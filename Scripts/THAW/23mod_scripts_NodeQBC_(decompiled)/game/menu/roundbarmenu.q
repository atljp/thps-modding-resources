...
script add_roundbar_menu_item {focus_script = roundbar_menu_item_focus
    ...
    if GotParam not_focusable
        ...
        CreateScreenElement {
            ...
            rot_angle = <rot>
            event_handlers = [
                {focus <focus_script> params = {id = <id> scale_highlight_text = <scale_highlight_text> show_bar = <show_bar>}}
                {unfocus <unfocus_script> params = {id = <id> scale_highlight_text = <scale_highlight_text> show_bar = <show_bar>}}
    ...
    CreateScreenElement {
        ...
        rgba = <rgba>
        Scale = <Scale>
        Pos = (<Pos> + (0.0, 3.0) + <internal_text_offset>)
        just = <just>
        internal_just = <just>
    }
    ...
    CreateScreenElement {
        ...
        Pos = (0.0, 3.0)
        just = [ left center ]
        Scale = (<right_edge> * 0.03100 + <y_scale>)
...
script roundbar_menu_highlight 
    ...
    DoScreenElementMorph id = <id> Pos = (<end_pos> + (0.0, 3.0))time = 0.1500
    DoScreenElementMorph id = <id> alpha = 0.8000 time = 0.3000
    wait 0.4000 seconds
    SetScreenElementProps {
        id = <id>
        Pos = (<right_edge> / 2 + (0.0, 3.0))
    }
    DoScreenElementMorph id = <id> Scale = (<width> * (0.0949999988079071, 0.0) + (0.0, 1.0))time = 0
    begin 
    DoScreenElementMorph id = <id> alpha = 1 time = 0.2000
    wait 0.2000 seconds
    DoScreenElementMorph id = <id> alpha = 0 time = 0.2000
    wait 0.2000 seconds
    repeat 
endscript
...
script #"0x04075f7e" build_pause_parts = 1
    if (<build_pause_parts>)
        build_pause_menu_parts <...> 
    endif
    if ScreenElementExists id = music_track_anchor
