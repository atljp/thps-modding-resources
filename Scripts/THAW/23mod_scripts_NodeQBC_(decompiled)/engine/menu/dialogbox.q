...
script create_snazzy_dialog_box {Title = 'Tytu³'
        text = 'Domylny tekst'
    ...
    if (<font> = dialog)
        font = small
        printf '!!!'
        printf 'Dialog font looks like ass in snazzy dialog boxes. Tell Stuart this box needs updating'
        printf '!!!'
    endif
    ...
    if GotParam logo
        ...
        CreateScreenElement {
            ...
            Scale = ((1.399999976158142, 1.2999999523162842) * 1.400)
        }
        no_icon = no_icon
        if (<logo> = dnas)
            CreateScreenElement {
                type = TextBlockElement
                parent = <vmenu_id>
                text = '\nDNAS logo jest zastrze¿onym znakiem handlowym \nSony Computer Entertainment Inc.'
                font = dialog
                dims = (420.0, 0.0)
                allow_expansion
                just = [ center top ]
...
script animate_special_dialog_box_in 
    ...
    DoMorph \{Pos = (320.0, 260.0) alpha = 1 time = 0.3000 Scale = 1.0 rot_angle = 0}
endscript
...
script burst_star 
    ...
    DoMorph \{ time = 0.4000 Scale = (7.5, 3.799999952316284) alpha = 0.6000 }
endscript

script pulse_dialog_title 
    begin 
    if ObjectExists id = <id>
        DoScreenElementMorph id = <id> time = 0.2000 Scale = (1.7999999523162842, 2.0) rgba = [ 13 121 4 128 ]
        wait 0.2000 seconds
    else
        break 
    endif
    if ObjectExists id = <id>
        DoScreenElementMorph id = <id> time = 0.2000 Scale = (1.5, 1.5) rgba = [ 13 121 4 128 ]
        wait 0.2000 seconds
    else
        break 
    endif
...
script build_theme_dialog_bottom 
    ...
    if NOT GotParam no_icon
        ...
        if GotParam add_loading_anim
            Theme_GetHighlightedTextColorNew return_value = loading_color
        else
...
script create_speech_box {Pos = (320.0, 450.0)
    ...
    if GotParam pad_option_script
        SetScreenElementProps {
            id = <anchor_id>
            event_handlers = [{ pad_option speech_box_input_debounce }
...
script speech_box_input_debounce 
    ...
    Debounce \{ Circle time = 0.3000 Clear = 1 }
    Debounce \{ square time = 0.3000 Clear = 1 }
    Debounce \{ triangle time = 0.3000 Clear = 1 }
endscript
...
script create_test_dialog 
    ...
    create_dialog_box \{pad_back_script = create_test_menu buttons = [{ font = small text = 'Nie' pad_choose_script = create_test_menu }{ font = small text = 'Tak' pad_choose_script = null_script }]}
endscript
