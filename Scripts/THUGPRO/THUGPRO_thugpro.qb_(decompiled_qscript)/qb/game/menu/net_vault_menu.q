...
SCRIPT create_downloads_menu 
	...
	make_new_menu { menu_id = downloads_menu vmenu_id = downloads_vmenu internal_just = [ center center ] 
	pos = PAIR(400.00000000000, 115.00000000000) } 
	SetScreenElementProps { id = downloads_menu event_handlers = [ { pad_back launch_new_ss_menu params = { from_ss_menu } } ] } 
	AssignAlias id = downloads_vmenu alias = current_menu 
	create_menu_background_title title = "downloads" bg_just = right no_grain 
	create_helper_text generic_helper_text 
	make_thpg_menu_item text = "" not_focusable = not_focusable 
	make_thpg_menu_item { text = "parks" id = menu_down_parks pad_choose_script = download_content pad_choose_params = { type = parks } } 
	make_thpg_menu_item { text = "skater" id = menu_down_skaters pad_choose_script = download_content pad_choose_params = { type = skaters } } 
	make_thpg_menu_item { text = "tricks" id = menu_down_tricks pad_choose_script = download_content pad_choose_params = { type = tricks } } 
	make_thpg_menu_item { text = "goals" id = menu_down_goals pad_choose_script = download_content pad_choose_params = { type = goals } } 
...
SCRIPT create_uploads_menu 
	...
	CreateScreenElement { 
		...
		parent = uploads_anchor 
		texture = white2 
		scale = PAIR(33.00000000000, 5.00000000000) 
...
SCRIPT net_vault_menu_create 
	...
	ELSE 
		title = "NEVERSOFT VAULT" 
	ENDIF 
	build_theme_sub_title title = <title> no_grad_bar 
	...
	CreateScreenElement { 
		...
		pos = PAIR(35.00000000000, 65.00000000000) 
		just = [ left top ] 
		rgba = [ 0 0 0 128 ] 
		alpha = 0.80000001192 
		z_priority = 2 
...
SCRIPT net_vault_menu_add_tag 
	...
	ELSE 
		<anchor_id> : SetTags description = "Description goes here... oops!" 
	ENDIF 
	net_vault_menu_add_text anchor_id = <anchor_id> text = <name> pos = PAIR(0.00000000000, 0.00000000000) 
	net_vault_menu_add_text anchor_id = <anchor_id> text = <creator> pos = PAIR(220.00000000000, 0.00000000000) 
	IF GotParam focus 
...
SCRIPT net_vault_menu_focus 
	...
	IF GotParam theme 
		IF ScreenElementExists id = theme_text 
			SWITCH <theme> 
				CASE 0 
					SetScreenElementProps id = theme_text text = "Antarctica" 
				CASE 1 
