...
SCRIPT high_scores_menu_create 
	...
	IF GotParam from_options 
		...
		SetScreenElementProps { id = sub_menu 
			event_handlers = [ { pad_choose generic_menu_pad_back params = { callback = high_scores_menu_exit from_options no_unload_textures } } 
				{ pad_start generic_menu_pad_back params = { callback = high_scores_menu_exit from_options no_unload_textures } } 
		...
		create_helper_text generic_helper_text_view_menu 
		CreateScreenElement { 
			type = SpriteElement 
			parent = sub_menu 
			texture = black 
	...
	CreateScreenElement { 
		...
		pos = <pos> 
		rgba = ( ( THUGPRO_GlobalThemeInfo ) . ICON_ON_VALUE ) 
		just = [ left top ] 
		z_priority = 1 
	} 
	CreateScreenElement { 
