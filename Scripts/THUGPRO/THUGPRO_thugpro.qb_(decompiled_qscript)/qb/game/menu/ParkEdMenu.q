...
temp_parked_height_patch_disabled = 0 
RAILTOOL_IS_ACTIVE = 0 
SCRIPT parked_piece_focus_script 
	...
	IF ( ( <id> = Sk3Ed_GAP_01 ) | ( <id> = Sk5Ed_Sel_01 ) ) 
		Change PARKED_NO_VIEWERCAM = 1 
	ELSE 
		Change PARKED_NO_VIEWERCAM = 0 
	ENDIF 
	IF ( <id> = Sk3Ed_GAP_01 ) 
	...
	ELSE 
		wait 5 gameframe 
		IF IsTrue temp_parked_height_patch_disabled 
			THUGPRO_ParkEdHeightPatch on 
			Change temp_parked_height_patch_disabled = 0 
		ENDIF 
	ENDIF 
	IF ( <id> = rail_placement ) 
		Change RAILTOOL_IS_ACTIVE = 1 
		SetScreenElementProps { 
			id = rail_placement 
			event_handlers = [ 
				{ pad_r3 RailEditor_ToggleGrid params = { } } 
				{ pad_l3 RailEditor_TogglePost params = { } } 
				{ focus parked_piece_focus_script } 
				{ unfocus parked_piece_unfocus_script } 
			] 
			replace_handlers 
...
SCRIPT parked_set_helper_text_mode 
	...
	SWITCH <mode> 
		CASE regular 
			create_helper_text park_editor_helper_text parent = piece_menu_container helper_pos = parked_helper_text_pos scale = 0.89999997616 no_bar 
		CASE gap_regular 
			create_helper_text gap_regular_helper_text parent = piece_menu_container helper_pos = parked_helper_text_pos scale = 0.89999997616 no_bar 
		CASE gap_adjust 
			create_helper_text gap_adjust_helper_text parent = piece_menu_container helper_pos = parked_helper_text_pos no_bar 
		CASE rail_placement 
			...
			SWITCH <mode> 
				...
				CASE Grab 
					...
					<scale> = 0.89999997616 
			ENDSWITCH 
			create_helper_text <helper_text> parent = piece_menu_container helper_pos = parked_helper_text_pos scale = <scale> no_bar 
		DEFAULT 
	ENDSWITCH 
...
SCRIPT parked_update_percent_bar 
	...
	ELSE 
		SetScreenElementProps id = percent_bar_colored_part rgba = ( ( THUGPRO_GlobalThemeInfo ) . HIGHLIGHTED_TEXT_COLOR ) 
	ENDIF 
...
SCRIPT parked_make_set_menu_item 
	...
	theme_menu_add_item text = <set_name> text_just = [ center center ] no_sound no_bg highlight_bar_scale = PAIR(1.50000000000, 0.69999998808) 
