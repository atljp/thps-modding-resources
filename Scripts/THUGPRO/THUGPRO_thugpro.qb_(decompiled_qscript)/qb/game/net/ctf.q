...
SCRIPT goal_ctf_activate 
	...
	IF ( <unlimited_time> = 1 ) 
		IF ( <score> = 1 ) 
			FormatText TextName = msg_text "First to %s capture wins" s = <score> 
		ELSE 
			FormatText TextName = msg_text "First to %s captures wins" s = <score> 
		ENDIF 
	ELSE 
		FormatText TextName = msg_text "Most captures wins" 
	ENDIF 
...
SCRIPT destroy_ctf_panel_message 
	IF ObjectExists id = net_panel_msg_bg 
