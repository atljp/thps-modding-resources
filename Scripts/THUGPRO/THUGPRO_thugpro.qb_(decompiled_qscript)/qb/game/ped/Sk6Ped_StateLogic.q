...
SCRIPT STATECONTROL_fall 
	IF SkaterSpeedLessThan 1 
		BEGIN 
			RANDOM(3, 1) RANDOMCASE 
				ped_play_idle_anim RANDOMCASE 
				ped_play_disgust_anim no_block 
			Ped_Utils_PlayStream Type = warning RANDOMEND 
			Obj_WaitAnimFinished 
...
SCRIPT STATECONTROL_knockdown 
	...
	ENDIF 
	Ped_Utils_PlayStream Type = warning 
	Obj_WaitAnimFinished 
	ped_rotate_to_skater_from_idle 
	Ped_SetLogicState STATE_disgust Table = Ped_StateLogic_Table 
ENDSCRIPT
...
SCRIPT STATECONTROL_avoidland 
	...
	BEGIN 
		...
		IF ( <stopped> = 1 ) 
			ped_rotate_to_skater_from_idle 
			RANDOM(1, 1, 1) RANDOMCASE 
				ped_play_disgust_anim 
			Ped_Utils_PlayStream Type = warning RANDOMCASE ped_play_idle_anim RANDOMCASE ped_play_idle_anim RANDOMEND 
		ELSE 
			IF NOT ( <move_type> = stand ) 
...
SCRIPT STATECONTROL_getattention 
	...
	BEGIN 
		...
		ENDIF 
		GetSingleTag __STOP_TALKING 
		IF GotParam __STOP_TALKING 
			IF ( <__STOP_TALKING> = 0 ) 
...
SCRIPT STATECONTROL_Sitting 
	...
	BEGIN 
		...
		RANDOM(1, 1, 1, 1, 1) 
			...
			OBJ_cycleanim Anim = Ped_SIT_Idle NumCycles = <RepeatIdle> 
			...
			BEGIN 
				RANDOM(1, 1, 1) 
					...
					OBJ_cycleanim Anim = Ped_sitB_idle NumCycles = <RepeatIdleB> 
			...
			BEGIN 
				RANDOM(1, 1, 1, 1, 1) 
					...
					OBJ_cycleanim Anim = Ped_sitBL_idle NumCycles = <RepeatIdleBL> 
