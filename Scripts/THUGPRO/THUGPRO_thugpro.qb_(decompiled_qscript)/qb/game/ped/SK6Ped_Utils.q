	...
	IF GotParam VO 
		IF ObjectExists id = <ped_id> 
			...
			ELSE 
				Printf "" 
				Printf "############################################" 
				Printf "##  Trying to play a hint ped stream...   ##" 
				Printf "##                                        ##" 
				Printf "##  %v" v = <stream> 
				Printf "##                                        ##" 
...
SCRIPT Ped_Utils_PlayStream 
	IF GotParam Type 
		...
		ELSE 
			IF GotParam Type 
				IF ChecksumEquals a = <Type> b = Far 
			...
			IF GotParam VO_Types 
				IF StructureContains <Type> structure = <VO_Types> 
					IF IsArray ( <VO_Types> . <Type> ) 
						IF GotParam index 
							Obj_PlayStream ( <VO_Types> . <Type> [ <index> ] ) vol = 200 
						ELSE 
							GetRandomArrayElement ( <VO_Types> . <Type> ) 
							Obj_PlayStream ( <element> ) vol = 200 
						ENDIF 
					ELSE 
						Obj_PlayStream ( <VO_Types> . <Type> ) vol = 200 
					ENDIF 
				ENDIF 
			ENDIF 
		ENDIF 
