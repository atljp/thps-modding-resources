...
Trick_BSFootplant = { Scr = LipMacro2 Params = { Name = #"BS Boneless" Score = 550 InitAnim = BSFootplant_init Anim = BSFootplant_range OutAnim = BSFootplant_out OllieOutAnim = BSFootplant_OllieOut FlipAfter UseBoneless ReleaseFrame = 20 BailScript = NoseManualBail RevertBS ExtraTricks = LipTrickExtras } } 
...
Trick_Russian = { Scr = LipMacro2 Params = { Name = #"Russian Boneless" Score = 6500 InitAnim = BSFootplant_init PingPong Anim = Russian_Idle OutAnim = Russian_out OllieOutAnim = Russian_OllieOut FlipAfter UseBoneless ReleaseFrame = 20 BailScript = NoseManualBail RevertBS ExtraTricks = LipTrickExtras IsSpecial } } 
Trick_HoHoSadplant = { Scr = LipMacro2 Params = { Name = #"Ho Ho Sad Plant" Score = 6750 InitAnim = HoHoSadPlant_Init PingPong Anim = HoHoSadPlant_Idle OutAnim = HoHoSadPlant_Out IsSpecial NoOllie } } 
Trick_HeelflipFSInvert = { Scr = LipMacro2 Params = { Name = #"Heelflip FS Invert" Score = 6500 InitAnim = HeelflipFSInvert_init Anim = HeelflipFSInvert_range OutAnim = HeelflipFSInvert_out NoOllie IsSpecial FlipAfter RevertBS ExtraTricks = InvertExtras } } 
Trick_Around = { Scr = LipMacro2 Params = { Name = #"Around The World" Score = 7250 Speed = 0.94999998808 InitAnim = Special_MikeV_Around_int Anim = Special_MikeV_Around_range OutAnim = Special_MikeV_Around_out IsSpecial NoBalance ReleaseFrame = 15 NoOllie } } 
Trick_BamLip = { Scr = LipMacro2 Params = { Name = #"BAM" Score = 6750 Speed = 0.85000002384 InitAnim = Special_Bam_Lip_Init Anim = Special_Bam_Lip_Idle OutAnim = Special_Bam_Lip_Out IsSpecial PlayBalanceAnimOnce ReleaseFrame = 20 RotateOnOllie ExtraTricks = LipTrickExtras } } 
...
SCRIPT LipMacro2 Speed = 1.00000000000 
	IF BailIsOn 
		SetState Air 
		Goto LipBail 
	ENDIF 
		...
		IF GotParam NoOllie 
			IF GotParam InitAnim 
				IF NOT GotParam Isextra 
					Playanim Anim = <InitAnim> Speed = <Speed> 
...
SCRIPT PlayLipBalancingAnim 
	IF NOT GotParam NoBalance 
		...
		IF GotParam PingPong 
			Playanim Anim = <Anim> PingPong Speed = <Speed> 
		ELSE 
			Playanim Anim = <Anim> wobble from = end To = Start Speed = <Speed> 
		ENDIF 
		IF GotParam Idle 
			Playanim Anim = <Anim> cycle Speed = <Speed> 
		ENDIF 
	ELSE 
		IF GotParam PlayBalanceAnimOnce 
			Playanim Anim = <Anim> Speed = <Speed> 
		ENDIF 
	ENDIF 
ENDSCRIPT
...
SCRIPT LipOut y = 1 
	...
	IF GotParam OutAnim 
		Playanim Anim = <OutAnim> Blendperiod = 0.00000000000 Speed = <Speed> 
		BlendPeriodOut 0.30000001192 
	ELSE 
		IF GotParam InitAnim 
			Playanim Anim = <InitAnim> backwards Speed = <Speed> 
		ELSE 
		ENDIF 
	ENDIF 
...
SCRIPT LipTrick 
	...
	ClearTrickQueue 
	KillExtraTricks 
	IF NOT IsTrue ENABLE_LIP_SPINS 
		CancelRotateDisplay 
	ENDIF 
	IF Obj_FlagSet FLAG_SKATER_LIPCOMBO 
