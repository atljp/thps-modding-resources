...
print_trick_info = 0 
balance_check = 0 
umbrella_float = 0.00000000000 
NoLOD = 0 
GenerateDefaultModelCollData = 0 
bonecollisioninfo = [ 
	{ 
		bone = Bone_head 
		radius = 3.00000000000 
	} 
] 
use_auto_menu_input = 0 
SCRIPT auto_menu_input 
ENDSCRIPT
...
scripts_to_kill_when_changing_levels = [ 
	...
	skater_rotate_wheel_test 
	thugpro_wallplant_check 
] 
SCRIPT PrintState 
	IF InAir 
		printf "In air" 
...
SCRIPT change_level 
	...
	IF NOT IsObserving 
		GetSkaterId 
		<ObjId> : StatsManager_DeactivateGoals 
	ENDIF 
	...
	IF InSplitScreenGame 
		launch_two_player 
	ENDIF 
	IF IsTrue is_quitting 
		SkProDisplayLoadingScreen freeze 
	ENDIF 
	printf "ChangeLevel end" 
	ChangeLevel <...> 
ENDSCRIPT
...
SCRIPT StartAirGap 
	StartGap flags = [ CANCEL_GROUND CANCEL_WALL CANCEL_MANUAL CANCEL_LIP CANCEL_RAIL ] <...> 
ENDSCRIPT

SCRIPT StartRailGap 
	StartGap flags = [ CANCEL_GROUND CANCEL_WALL CANCEL_MANUAL CANCEL_LIP CANCEL_AIR ] <...> 
ENDSCRIPT

SCRIPT StartManualGap 
	StartGap flags = [ CANCEL_GROUND CANCEL_WALL CANCEL_LIP CANCEL_RAIL CANCEL_AIR ] <...> 
ENDSCRIPT

SCRIPT StartWallGap 
	StartGap flags = [ CANCEL_GROUND CANCEL_MANUAL CANCEL_LIP CANCEL_RAIL CANCEL_AIR ] <...> 
ENDSCRIPT
...
SCRIPT ped_speech_accept2 
	...
	IF GotParam activation_script 
		IF NOT GotParam colObjId 
			<colObjId> : Obj_SpawnScript <activation_script> Params = { ped_id = <ped_id> <activation_script_params> } 
		ELSE 
			Skater : Obj_SpawnScript <activation_script> Params = { ped_id = <ped_id> <activation_script_params> } 
		ENDIF 
	ENDIF 
...
have_loaded_thugpro_permanent = 0 
have_loaded_permanent = 0 
have_loaded_unloadable = 0 
have_loaded_net = 0 
SCRIPT do_load_thugpro_permanent 
	IF ( have_loaded_thugpro_permanent ) 
	ELSE 
		PushMemProfile "Permanent THUGPRO Anims" 
		MemPushContext SkProPermanent 
		SetDefaultPermanent 1 
		PushMemProfile "Permanent THUGPRO Anim Pipped PRE" 
		LoadPipPre "thugpro_anims.pre" 
		PopMemProfile 
		load_permanent_thugpro_anims use_pip 
		SetDefaultPermanent 0 
		MemPopContext 
		PopMemProfile 
		change have_loaded_thugpro_permanent = 1 
	ENDIF 
ENDSCRIPT

SCRIPT do_unload_thugpro_permanent 
	IF ( have_loaded_thugpro_permanent ) 
		load_permanent_thugpro_anims LoadFunction = UnloadAnim 
		IF NOT UnloadPipPre "thugpro_anims.pre" 
		ENDIF 
		change have_loaded_thugpro_permanent = 0 
	ENDIF 
ENDSCRIPT
...
SCRIPT restore_start_key_binding 
	IF IsTrue bootstrap_build 
		SetScreenElementProps { 
			id = root_window 
			event_handlers = [ { pad_start THUGPRO_Bootstrap_Menu } ] 
			replace_handlers 
		} 
	ELSE 
		SetScreenElementProps { 
