...
SCRIPT client_toggle_cheat 
	...
	ENDIF 
	IF ArrayContains array = ( SERVER_LEVEL_FLAGS ) contains = <cheat_flag> 
		IF ( ( ( LEVEL_ORIGINAL_GAME ) = THPS4 ) | ( LevelIs Load_NY ) ) 
			HandleProtrickObjects cheat_flag = <cheat_flag> 
		ENDIF 
		HandleLevelOptionObjects 
	ENDIF 
	IF ArrayContains array = ( SERVER_FLAGS_GLOBAL_FLAGS ) contains = <cheat_flag> 
		HandleServerGlobalFlags <...> 
	ENDIF 
ENDSCRIPT

SCRIPT CreateServerRemovedYouDialog_Mods 
	HideLoadingScreen 
	...
	create_error_box { 
		...
		text = "The server does not allow mods, and has chosen to remove you from this game. Press \\bm to return to the lobby." 
