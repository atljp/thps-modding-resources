		...
		CASE ngc 
			...
			create_error_box { 
				...
				buttons = [ 
					{ font = small text = #"Continue without saving" pad_choose_script = reload_anims_then_run_abort_script } 
					{ font = small text = #"Retry" pad_choose_script = RetryScript } 
					{ font = small text = #"Delete/Overwrite THUG 2 files" pad_choose_script = launch_files_menu pad_choose_params = { DoNotShowNotEnoughRoomMessage Save FileType = <FileType> } } 
				] 
			} 
	ENDSWITCH 
ENDSCRIPT
...
SCRIPT mcmess_NoFiles 
	...
	SWITCH <Platform> 
		CASE ps2 
			FormatText { 
				TextName = text 
				#"No THUG 2 %n save data present on memory card (8MB) (for PlayStation\xAE2) in MEMORY CARD slot 1." 
				n = <filetype_name> 
			} 
			create_memcard_error_dialog_box { 
				title = "Warning !" 
		...
		CASE xbox 
			FormatText { 
				TextName = text 
				#"No THUG 2 %n present on hard disk." 
				n = <filetype_name> 
			} 
			memcard_menus_cleanup 
			create_error_box { 
		...
		CASE ngc 
			FormatText { 
				TextName = text 
				#"No THUG 2 %n data present on Nintendo GameCube Memory Card in Slot A." 
				n = <filetype_name> 
			} 
			create_memcard_error_dialog_box { 
				title = "Warning !" 
...
SCRIPT mcmess_ErrorNotFormatted QuitText = #"Continue without formatting" BackScript = reload_anims_then_run_abort_script 
	...
	SWITCH <Platform> 
		CASE ps2 
			create_error_box { 
				text_dims = PAIR(600.00000000000, 0.00000000000) 
				title = #"Warning !" 
		...
		CASE ngc 
			IF CardIsForeign 
				create_error_box { 
					text_dims = PAIR(590.00000000000, 0.00000000000) 
					title = #"Warning !" 
	...
	SWITCH <Platform> 
		CASE ps2 
			...
			create_error_box { 
				title = #"Warning !" 
				text = <text> 
				text_dims = PAIR(580.00000000000, 0.00000000000) 
				pos_tweak = PAIR(0.00000000000, -33.00000000000) 
				no_animate 
				pad_back_script = reload_anims_then_run_abort_script 
				buttons = [ 
		...
		CASE ngc 
			...
			IF GotParam no_manager 
				create_error_box { 
					title = #"Warning !" 
					text = <text> 
					text_dims = PAIR(550.00000000000, 0.00000000000) 
					pos_tweak = PAIR(15.00000000000, -17.00000000000) 
					no_animate 
					pad_back_script = reload_anims_then_run_abort_script 
					buttons = [ 
						{ font = small text = #"Continue without saving all types of files" pad_choose_script = reload_anims_then_run_abort_script } 
	...
	SWITCH <Platform> 
		...
		CASE ngc 
			...
			create_error_box { 
				...
				pos_tweak = PAIR(0.00000000000, 0.00000000000) 
				no_animate 
				pad_back_script = generic_menu_pad_back 
				pad_back_params = { callback = reload_anims_then_run_abort_script } 
				buttons = [ 
