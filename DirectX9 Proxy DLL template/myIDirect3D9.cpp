...
HRESULT __stdcall myIDirect3D9::CreateDevice(UINT Adapter,D3DDEVTYPE DeviceType,HWND hFocusWindow,DWORD BehaviorFlags,D3DPRESENT_PARAMETERS* pPresentationParameters,IDirect3DDevice9** ppReturnedDeviceInterface)
{
	...
	//hFocusWindow = CreateWindowExA( // SOMEHOW SOMEWHERE );

	CreateFullscreenWindow(hFocusWindow);
	...
	SetWindowLongPtr(hFocusWindow, GWL_STYLE, WS_OVERLAPPEDWINDOW);
	SetWindowPos(hFocusWindow, HWND_TOP, 100, 100, R.right, R.bottom, SWP_NOZORDER | SWP_SHOWWINDOW);
	//----------------------------------------------------------------------------------------
	...
	*ppReturnedDeviceInterface = gl_pmyIDirect3DDevice9;
	...
	SetWindowTextA(hFocusWindow, "MYANMODv0.6");
	gl_pmyIDirect3DDevice9->Reset(pPresentationParameters);
	gl_pmyIDirect3DDevice9->ShowCursor(TRUE);
	//gl_pmyIDirect3DDevice9->SetCursorPosition(0, 0, D3DCURSOR_IMMEDIATE_UPDATE);
