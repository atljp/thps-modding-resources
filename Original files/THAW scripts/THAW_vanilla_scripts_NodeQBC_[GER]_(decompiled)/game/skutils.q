...
script play_vo_sound 
    ...
    if (<goal> = -1)
        printstruct <...> 
        scriptassert 'Could not find goal voice over type'
    endif
    ...
    if ObjectExists id = ped_speech_dialog
        ...
        if (<show_speech_box> = 1)
            ...
            if NOT GotParam accept_text
                ...
                FormatText textname = accept_text '%s: \me zum Reden' s = <display_name>
            endif
            if ObjectExists id = ped_speech_dialog
                DestroyScreenElement id = ped_speech_dialog
            endif
...
script Integrated_CAS_Ped_Script type = barbershop
    ...
    switch <type>
        ...
        setup_ped_speech {
            ...
            ped_id = <ped_id>
            text = 'Drück\m5, um Frisur zu wählen.'
        ...
        setup_ped_speech {
            dont_deactivate_goals
            ped_id = <ped_id>
            text = 'Drück\m5, um eine Grafik zu erstellen.'
...
script Integrated_CAGR_Launch 
    ...
    if skater ::IsSkaterOnBike
        ...
        create_snazzy_dialog_box {
            Title = 'Sorry'
            text = 'Auf dem Bike kannst du nichts markieren.'
            text_dims = (400.0, 0.0)
    ...
    if NOT InFrontend
        ...
        if ((IsXenon)|| (IsXbox)|| (isPC))
            <text> = 'Alle hier vorgenommenen Änderungen haben KEINEN Einfluss auf die im Story-Modus erstelle Grafik.\n\nDie im Story-Modus erstellte Grafik kann nur geändert werden, indem du während des Spiels mit dem Sprüher redest.\n\nIn diesem Menü kannst du die Grafiken bearbeiten, die in den Modi Klassik, Highscore und 2 Spieler verwendet werden.'
        else
            if IsNGC
                <text> = 'Alle hier vorgenommenen Änderungen haben KEINEN Einfluss auf die im Story-Modus erstelle Grafik.\n\nDie im Story-Modus erstellte Grafik kann nur geändert werden, indem du während des Spiels mit dem Sprüher redest.\n\nIn diesem Menü kannst du die Grafiken bearbeiten, die in den Modi Klassik, Highscore und 2 Spieler verwendet werden.'
            else
                <text> = 'Sämtliche hier vorgenommenen Änderungen haben KEINEN Einfluss auf die im Story-Modus erstellte Grafik.\n\nDie im Story-Modus erstellten Grafiken können nur geändert werden, indem du während des Spiels mit dem Sprüher redest.\n\nHier kannst du die Grafiken bearbeiten, die in den Modi Klassik, Highscore, 2 Spieler und Online verwendet werden.'
            endif
        endif
        create_snazzy_dialog_box {
            Title = 'HINWEIS!'
...
script Integrated_CAT_Ped_Script 
    ...
    setup_ped_speech {
        ped_id = <ped_id>
        accept_text = 'Drück\ms, um einen Trick zu erstellen'
...
script goal_check_goal_already_active_box_create 
    ...
    create_error_box {Title = 'Hey'
        text = 'Du solltest erst deine Mission beenden.'
        text_dims = (400.0, 0.0)
        bg_scale = 1.200
        buttons = [
...
script THUG3_Ped_Bike_Guy 
    ...
    setup_ped_speech {
        ped_id = <ObjID>
        accept_text = 'Drück \ms um Rad zu fahren'
...
script enter_telescope_cam {Pos = (-28542.900390625, 1270.300048828125, 10882.7001953125)
    ...
    create_helper_text parent = exit_telescope_anchor helper_text_elements = [{ text = '\m1=Ende' }] no_bar
    CreateScreenElement {
        type = SpriteElement
...
script trampoline_bounce max_vel = 1500
    ...
    if Held x
        if NOT heldlongerthan button = x 0.5000 second
            printf 'Good Bounce'
            GetGap GapID = shaba_bounce text = 'Bounce' Score = 10
    ...
    if onrail
        ...
        if NOT ((<tmpArray> [ <skaternumber> ])= 1)
            if ((<tmpArray> [ <skaternumber> ])= 2)
                FormatText textname = THAW_Grind_Counter_Text '%a Runde' a = ((<tmpArray> [ <skaternumber> ])-1)
            else
                FormatText textname = THAW_Grind_Counter_Text '%a Runden' a = ((<tmpArray> [ <skaternumber> ])-1)
            endif
            if InSplitScreenGame
...
script THAW_skater_laydown 
    ...
    setup_ped_speech {
        ped_id = <nodeName>
        accept_text = 'Drück \ms zum Pennen!'
...
script THAW_skater_passtime_controller 
    ...
    if GoalManager_HasActiveGoals
        text = 'Schließ deine Mission ab, bevor du dich ausruhst!'
    endif
    if NOT skater ::walking
        text = 'Du musst gehen, bevor du dich ausruhst!'
    endif
    if skater ::IsSkaterOnBike
        text = 'Steig vom Bike ab, bevor du dich ausruhst!'
    endif
    if skater ::currentscorepotgreaterthan 0
        text = 'Schließ deine Kombo ab, bevor du dich ausruhst!'
    endif
    if GotParam text
        GoalManager_PauseAllGoals
        PauseSkaters
