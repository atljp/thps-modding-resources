...
script trick_got_idle 
    if IsArray <Trick>
        if StructureContains structure = (<Trick> [ 0 ])params
            ...
            printstruct <...> 
            script_assert text = 'trick_got_idle was passed a trick that doesn\'t have a global structure'
        endif
    else
...
script setup_cat_manual 
    ...
    LaunchSpecialMessage text = 'Spezial-Manual'
    if (<name> = '')
        name = 'Erstellter Trick'
    endif
    SetTrickName <name>
...
script cat_unload_and_exit_pause 
    ...
    if GotParam trick_button_text
        ...
        <button_text> = ['Jawohl!!' 'Juhu!' 'Wahnsinn!!' 'Yeah, Baby!' 'Super!' 'Schön!' 'Krass!' 'Die Party geht weiter!' 'Boah!']
        GetArraySize button_text
        getrandomvalue name = button_text_index a = 0 b = (<array_size> - 1)integer
        create_snazzy_dialog_box {
            Title = 'Erstellter Spezialtrick'
    ...
    if ScreenElementExists id = root_window
        ...
        if GotParam next_dialog_text
            if NOT (<next_dialog_text> = '')
                ...
                GoalManager_HidePedFloaters
                create_snazzy_dialog_box {
                    Title = 'Verwende deinen neuen Trick.'
...
script auto_assign_cat_to_slot 
    ...
    if GotParam save_slot
        ...
        if (loading_cat_from_edit_tricks = 0)
            if ((IsXbox)|| (isPC))
                create_dialog_box {Title = 'Voll'
                    text = 'All deine Mach-dir-den-Trick-Slots sind voll. Was möchtest du tun?'
                    just = [ center center ]
                    text_rgba = [ 88 105 112 128 ]
                    buttons = [{font = small text = 'Alten Trick überschreiben' pad_choose_script = cat_assign_slot_menu}
                        {font = small text = 'Trick auf Festplatte speichern' pad_choose_script = launch_save_cat_sequence}
                        {font = small text = 'Trick nicht speichern' pad_choose_script = back_to_options_menu_from_cat}
                    ]
                }
            else
                create_dialog_box {Title = 'Voll'
                    ...
                    text_rgba = [ 88 105 112 128 ]
                    buttons = [{font = small text = 'Alten Trick überschreiben' pad_choose_script = cat_assign_slot_menu}
                        {font = small text = 'Trick speichern' pad_choose_script = launch_save_cat_sequence}
                        {font = small text = 'Trick nicht speichern' pad_choose_script = back_to_options_menu_from_cat}
                    ]
                }
            endif
            ...
            create_dialog_box {Title = 'Voll'
                ...
                text_rgba = [ 88 105 112 128 ]
                buttons = [{font = small text = 'Alten Trick überschreiben' pad_choose_script = cat_assign_slot_menu}
                    {font = small text = 'Verlassen ohne Trick zu laden' pad_choose_script = create_edit_tricks_menu}
                ]
            }
        endif
    endif
...
script cat_assign_slot_menu 
    dialog_box_exit
    make_new_themed_option_menu Title = 'TRICK ÜBERSCHREIBEN'
    if InFrontend
    endif
    if (loading_cat_from_edit_tricks = 0)
        SetScreenElementProps {
...
CATSFX_StripClub = {
    x
    params = { sound name = "Partymädel" anim = AirIdleCAT stream = NO_Female1_Flash05 }
}
CATSFX_Burp = {
    x
...
CATSFX_Fart = {
    x
    params = { sound name = "Schlechter Burrito" anim = AirIdleCAT stream = Fart01 }
}
CATSFX_Flush = {
    x
...
CATTrick_other04 = {
    x
    params = { name = "Jubel 1" anim = Cheer1 }
}
CATTrick_other05 = {
    x
    params = { name = "Jubel 2" anim = Cheer2 }
}
CATTrick_other06 = {
    x
    params = { name = "Schlampig" anim = SketchyLand }
}
CATTrick_other07 = {
    x
...
CATTrick_other08 = {
    x
    params = { name = "Spott 1" anim = Taunt1 }
}
CATTrick_other09 = {
    x
    params = { name = "Spott 2" anim = Taunt2 }
}
CATTrick_other10 = {
    x
    params = { name = "Spott 3" anim = Taunt3 }
}
CATTrick_other11 = {
    x
