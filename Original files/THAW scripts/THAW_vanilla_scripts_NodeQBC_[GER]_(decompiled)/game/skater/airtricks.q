...
ScavengerF = {
    scr = FlipTrick
    params = { name = "\c4Object Placed!\c0" Score = 0 anim = HeelFlip SH_Drop }
}
FireballF = {
    scr = FlipTrick
    params = {
...
script FlipTrick Speed = 1.0 trickslack = 10 grindslack = 25 flip_stat_mod = 1.0
    ...
    BlendPeriod = 0.2000
    ExtraBlendPeriod = 0.05000
    if GotParam specialitem_details
        OnExitRun TurnOffSpecialItem
    ...
    if innollie
        if GotParam Nollie
            if GotParam IsExtra
                Skater_Playanim anim = <Nollie> from = current BlendPeriod = <ExtraBlendPeriod> Speed = <Speed>
            else
                Skater_Playanim anim = <Nollie> BlendPeriod = <BlendPeriod> Speed = <Speed>
            endif
            GetAnimLength anim = <Nollie>
        else
            if GotParam IsExtra
                Skater_Playanim anim = <anim> from = current BlendPeriod = <ExtraBlendPeriod> Speed = <Speed>
            else
        ...
        if GotParam UseCurrent
            ...
            Skater_Playanim anim = <anim> from = current BlendPeriod = <BlendPeriod> Speed = <Speed>
        else
            Skater_Playanim anim = <anim> BlendPeriod = <BlendPeriod> Speed = <Speed>
        endif
    endif
    ...
    if AbilityEnabled doubletapflip
    ...
    if GotParam Spinslack
        if (<Length> > <Spinslack>)
            Skater_WaitAnim framesfromend = <Spinslack>
            canspin
        endif
    endif
    ...
    if GotParam grindslack
        if (<Length> * 60 > <grindslack>)
            Skater_WaitAnim framesfromend = <grindslack>
        endif
    endif
    bailoff
    if (<Length> * 60 > <trickslack>)
        ...
        Skater_WaitAnim framesfromend = 10
    endif
    if GotParam IsSpecial
        EndSpecial
    endif
