cas_store_params = {
    barbershop = {
        store_title = 'FRISEUR'
        text_length = 150
        accept_text = 'Drück \ms für eine neue\nFrisur'
        part = skater_m_hair
    ...
    clothingshop = {
        store_title = 'KLAMOTTEN-SHOP'
        text_length = 200
        accept_text = 'Drück \ms um dein Outfit zu wechseln'
    ...
    tattooshop = {
        store_title = 'TATTOO-SHOP'
        text_length = 150
        accept_text = 'Drück \ms zum Tätowieren'
    ...
    accessoryshop = {
        store_title = 'ZUBEHÖRLADEN'
        text_length = 150
        accept_text = 'Drück \ms um Klunker zu kaufen'
...
script enter_cas_store 
    if GameModeEquals is_career
        ...
        if skater ::IsSkaterOnBike
            text = 'Schaff das Bike hier raus!'
        else
            if NOT walking
                text = 'Skaten im Laden verboten!\n\n(Drück \mm zum Gehen)'
            else
                if currentscorepotgreaterthan 0
                    text = 'Beende deine Kombo, bevor du hier reinkommst!'
                else
                    <allow_enter> = 1
                    if NOT GoalManager_HasWonGoal name = Z_SM_Goal_Counter2
                        ...
                        if (<desc_id> = #"Alien Costume")
                            text = 'Leute wie dich bedienen wir hier nicht.'
                            <allow_enter> = 0
                        endif
                    endif
                    if (<allow_enter> = 1)
                        ...
                        if (<zone_name> = 'z_ho')
                            <allow_enter> = 0
                            <text> = 'Hey, du solltest mit Mindy reden.'
                        ...
                        if (<allow_enter> = 1)
                            if GoalManager_GoalExists name = (goal_utility_last_goal_activated)
                                if GoalManager_GoalIsActive name = (goal_utility_last_goal_activated)
                                    <allow_enter> = 0
                                    <text> = 'Du solltest deine Mission abschließen, bevor du shoppen gehst.'
                        ...
                        if (<allow_enter> = 1)
                            if GotParam type
                                ...
                                create_helper_text parent = exit_shop_anchor helper_text_elements = [{ text = '\m1=Shop verlassen' }] no_bar
...
script cas_store_enter 
    ...
    if NOT StructureContains structure = (cas_store_params.<type>)<level>
        scriptassert 'No level params found for this type of store'
    endif
...
script cas_store_exit 
    ...
    if NOT (cas_temp_shirt_desc = 0)
        create_snazzy_dialog_box \{Title = 'Hey' text = 'Willst du dein Shirt wieder anziehen?' text_dims = (400.0, 0.0) pad_back_script = NullScript no_bg buttons = [{ text = 'Ja' pad_choose_script = cas_store_replace_shirt }{ text = 'Nein' pad_choose_script = cas_store_exit_continue }]}
        return 
    endif
...
edit_skater_clothing_options = [
    ...
    { text = "Hosen" vo_text = 'pants_' submenu = pants_menu cam_anim_sub = Legs }
    {
        text = "Hosenfarbe"
...
edit_skater_tattoo_options = [
    {
        group_title = "TATTOO-SHOP"
        text = "Shirt entfernen"
        vo_text = 'tattoo_'
...
edit_skater_accessory_options = [
    {
        group_title = "ZUBEHÖRLADEN"
        vo_text = 'laccessory_'
        text = "Zubehör links"
        short_text = "Zubehör auswählen"
