...
script view_goals_progression_goals_get_list 
    ...
    if (view_goals_atom_shown = 0)
        view_goals_draw_atom text = 'Finde Teile für die Skate-Ranch.'
    endif
    if GetGlobalFlag flag = CAREER_SEEN_GAME_COMPLETE_MESSAGE
        view_goals_draw_atom text = 'Party auf der Ranch!'
    endif
...
script ProcessGoalList 
    ...
    if (<hide_goal_details> = 0)
        ...
        if IsArray <view_goals_info>
            ...
            <actionScript> goal = {view_goals_text = 'Dieser Charakter hat keine Ziele.' goal_id = blank}goal_list_index = <goal_list_index> not_focusable reminder_pos = <Pos>
        endif
    else
...
script view_goals_menu_build_bottom helper_text_elements = [{ text = '\b7/\b4 = Wählen' }
        { text = '\b6/\b5 = Zone' }
        { text = '\m0 = Missionsdetails' }
        { text = '\m1 = Zurück' }
    ]
    CreateScreenElement {
        type = ContainerElement
    ...
    if view_goals_root ::getsingletag simple_list
        if NOT view_goals_root ::getsingletag allow_description
            ...
            helper_text_elements = [{ text = '\m1/\m0 = Fertig' }]
        else
            if NOT GotParam goal
                helper_text_elements = [{ text = '\m1 = Fertig' }
                    { text = '\m0 = Missionsdetails' }
                ]
            endif
        endif
    ...
    if GotParam add_points
        ...
        FormatText textname = points1 '%c: %a / %b PKTE.' a = <active_points> b = <total_points> c = <level_name>
        Theme_GetAltColor return_value = points1_color
        FormatText textname = points2 'DU BRAUCHST NOCH %a PUNKTE, UM FORTZUFAHREN ...' a = <to_advance>
...
script view_goals_menu_create 
    ...
    if GotParam goal_text
        view_goals_menu_build_bottom helper_text_elements = [{ text = '\m1 = Zurück' }] goal
...
script viewgoals_create_scratches 
    ...
    if StructureContains structure = <goal> beaten
        if ((<goal>.reward_points)> 0)
            FormatText textname = points '%p PKTE.' p = (<goal>.reward_points)
            CreateScreenElement {
                type = TextElement
                parent = <container_id>
...
script view_goals_list_message 
    view_goals_generic_message \{message = '\cbDRÜCK \c0\m0\cb FÜR WEITERE INFOS' Pos = (450.0, 230.0) rgba = [ 128 128 128 128 ] rot = 0 Scale = (0.9800000190734863, 1.100000023841858)}
endscript

script view_goals_locked_message 
    kill_single_view_goals_cam
    FormatText textname = message '\cbFÜR NEUE ZIELE ZUM %a-CHARAKTER WECHSELN' a = ((skater_goal_list_names [ <goal_list_index> ]).name)
...
script view_created_goals_menu_create 
    ...
    create_helper_text helper_text_elements = [{ text = '\b7/\b4 = Wählen' }
        { text = '\m1 = Zurück' }
        { text = '\m0 = Sprung zum Ziel' }
    ]
    FormatText checksumname = title_icon '%i_gap' i = (THEME_PREFIXES [ current_theme_prefix ])
    build_theme_sub_title Title = 'ZIELE' title_icon = <title_icon>
    ...
    if GotParam group_num
        if (<active_group_num> = <group_num>)
            ...
            CreateScreenElement {
                ...
                parent = view_goals_stats
                font = small
                text = 'Neue Challenges verfügbar!'
                Pos = (70.0, 40.0)
                rot_angle = -0.5000
                rgba = [ 128 128 128 128 ]
                just = [ left center ]
        ...
        CreateScreenElement {
            ...
            parent = view_goals_stats
            font = small
            text = 'Hier gibt\'s keine Herausforderungen!'
            Pos = (70.0, 40.0)
            rot_angle = -0.5000
            rgba = [ 128 128 128 128 ]
            just = [ left center ]
