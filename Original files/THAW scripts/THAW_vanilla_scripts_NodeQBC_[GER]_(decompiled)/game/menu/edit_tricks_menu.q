...
script create_edit_tricks_menu 
    ...
    if (<tricks_editable> = 0)
        <menu_title> = 'TRICKS ANSEHEN'
    else
        <menu_title> = 'TRICKS BEARBEITEN'
    endif
    if NOT InFrontend
...
script create_edit_tricks_sub_menu 
    ...
    if (<type> = lip_tricks)
        ...
        <tap_info_text> = '1x drücken / 2x drücken'
    endif
...
script special_tricks_menu_add_special_to_mapping 
    ...
    if (<special_trickname> = Unassigned)
        <trick_display_text> = 'Nicht zugewiesen'
    else
        if (<iscat> = 1)
...
special_trick_types = [
    { type = FlipTrick heading = 'Air-Tricks' add_cats }
    { type = GrabTrick heading = 'Grab Tricks' add_cats }
    { type = LipMacro2 heading = 'Lip-Tricks' }
    { type = GrindTrick heading = 'Grind-Tricks' add_cats }
    { type = Manual heading = 'Manual-Tricks' add_cats }
]
