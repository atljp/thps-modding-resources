...
script classic_mode_process_intro_cameras show_beaten = 0
    ...
    if GotParam TimedScoreRun
        if GotParam just_unlocked
            ...
            view_goals_text = 'Du hast einen neuen Level freigespielt!'
...
script classic_mode_view_goals 
    ...
    container_id = <id>
    GetCurrentLevelName
    FormatText textname = Title '%a Ziele' a = <level_name>
    CreateScreenElement {
        type = TextElement
        parent = <container_id>
        shadow
...
script classic_mode_check_for_all_complete_message 
    ...
    if (<classic_level_goals> = 10)
        ...
        create_panel_message \{ id = all_goals_completed_message text = 'Alle Ziele abgeschlossen!' style = classic_goals_all_complete_message }
    endif
endscript
