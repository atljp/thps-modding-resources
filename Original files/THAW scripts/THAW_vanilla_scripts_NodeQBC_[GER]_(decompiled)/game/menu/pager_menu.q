    ...
    if InInternetMode
        if NetSessionFunc obj = presence func = profile_logged_in
            ...
            if (<num_notifications> > 0)
                NetSessionFunc obj = presence func = show_next_notification
            endif
        endif
    endif
...
script pager_window_build_menu 
    ...
    if NOT InInternetMode
        pager_window_add_menu_item text = 'ANTWORT-BUTTON' id = context_reply pad_choose_script = pager_window_context_reply
        pager_window_add_menu_item text = 'VORHERIGE NACHRICHT' id = prev pad_choose_script = pager_window_prev_message
    endif
    pager_window_add_menu_item text = 'NÄCHSTE NACHRICHT' id = next pad_choose_script = pager_window_next_message
    if InInternetMode
        if GotParam nick
            pager_window_add_menu_item text = 'ANTWORT' id = reply pad_choose_script = message_buddy pad_choose_params = { <...> from_buddy_message }
        endif
    endif
    pager_window_add_menu_item text = 'VERLASSEN' id = exit pad_choose_script = pager_window_destroy
    SpawnScriptLater SK6_SFX_IncomingBeeper
    PauseMusic 1
    CreateScreenElement {
        type = SpriteElement
        parent = pager_box_anchor
...
script pager_window_destroy 
    if ScreenElementExists id = pager_anchor
        FireEvent type = unfocus target = pager_anchor
        StartDelaySystemUI
...
script pager_window_update_message_title 
    if NOT InInternetMode
        if (<PagerMessageCount> = 0)
            ...
            FormatText textname = Title 'TEXTNACHRICHT %a:' a = <CurrentMessageNum>
        endif
    endif
...
script pager_window_update_message_pane 
    if InInternetMode
        ...
        if (<PagerMessageCount> = 0)
            ...
            if GotParam from
                FormatText textname = message 'VON: <%a>\n\n%b' a = <from> b = <message>
            endif
        endif
    endif
    ...
    if ScreenElementExists id = current_menu
        ...
        if GotParam reply_text
            ...
            current_menu ::settags reply_text = 'ANTWORT'
        endif
    endif
endscript
...
script pager_window_refresh_content 
    if InInternetMode
        FormatText textname = Title 'Nachricht von %s' s = <nick>
