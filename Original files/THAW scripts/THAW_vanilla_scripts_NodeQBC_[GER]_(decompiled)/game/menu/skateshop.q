...
script create_ss_menu 
    ...
    switch <gamemode>
        case career
        <menu_title> = 'STORY-MODUS'
        case freeskate
        if InSplitScreenGame
            if (<currentSkaterProfileIndex> = 1)
                <menu_title> = 'SPIELER ZWEI'
            else
                <menu_title> = 'SPIELER EINS'
            endif
        else
            <menu_title> = 'SKATE FOR FUN'
        endif
        case net
        if IsXbox
        ...
        case singlesession
        <menu_title> = 'EINZELSESSION'
        case classic
        <menu_title> = 'KLASSIK-MODUS'
        default 
    ...
    if GameModeEquals is_classic
        if (<currentSkaterProfileIndex> = 0)
            GetInitialsString
            FormatText textname = initials_string 'INITIALEN AKTUALISIEREN: %i' i = <string>
            add_label_menu_item {
...
script ss_overwrite_warning Title = 'Skater laden' callback = cas_reset_skater_and_goto_menu
    ...
    create_snazzy_dialog_box {
        ...
        buttons = [
            {font = small text = 'Nein' pad_choose_script = ss_cancel_load_skater}
            {
                font = small text = 'Ja'
