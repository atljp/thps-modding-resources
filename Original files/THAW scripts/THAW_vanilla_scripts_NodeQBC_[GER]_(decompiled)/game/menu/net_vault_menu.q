...
script net_vault_menu_create 
    ...
    if GotParam type
        switch <type>
            ...
            Title = 'PARKLAGER'
            case tags
            Title = 'MARKIERUNGSLAGER'
            case tricks
            Title = 'TRICKLAGER'
            case skaters
            Title = 'SKATERLAGER'
            default 
            Title = 'NEVERSOFT-LAGER'
        endswitch
    else
        Title = 'NEVERSOFT-LAGER'
    endif
...
script net_vault_menu_focus 
    ...
    if GotParam Score
        if ScreenElementExists id = score_text
            FormatText textname = score_string 'Punkte: %i' i = <Score>
    ...
    if GotParam is_male
        if ScreenElementExists id = sex_text
            if (<is_male> = 1)
                SetScreenElementProps id = sex_text text = 'Männlich'
            else
                SetScreenElementProps id = sex_text text = 'Weiblich'
            endif
        endif
    endif
    if GotParam num_pieces
        if ScreenElementExists id = pieces_text
            FormatText textname = string 'Teile: %i' i = <num_pieces>
    ...
    if GotParam theme
        if ScreenElementExists id = theme_text
            switch <theme>
                case 0
                SetScreenElementProps id = theme_text text = 'Antarktis'
                case 1
                SetScreenElementProps id = theme_text text = 'Mars'
                case 2
                SetScreenElementProps id = theme_text text = 'Einsame Insel'
                case 3
                SetScreenElementProps id = theme_text text = 'Lagerhalle'
                case 4
                SetScreenElementProps id = theme_text text = 'Gefängnishof'
            endswitch
        endif
    endif
