...
badversionnumber = "UNGÜLTIGE VERSION!"
damagedfile = "BESCHÄDIGT!"
NGCDamagedFile = "BESCHÄDIGTE DATEI!"
...
script GetFileTypeName 
    switch <file_type>
        case OptionsAndPros
        return filetype_name = 'SPIELFORTSCHRITT'
        case NetworkSettings
        if IsXbox
            return filetype_name = 'SYSTEM LINK EINSTELLUNGEN'
        else
            return filetype_name = 'ONLINE-EINSTELLUNGEN'
        endif
        case cas
        ...
        case CreatedGoals
        return filetype_name = 'ZIELE'
        case createdgraphic
        return filetype_name = 'GRAFIKEN'
        default 
...
script create_files_menu pos_tweak = (-20.0, -45.0)
    ...
    if GotParam save
        menu_title = 'SPEICHERN'
        AddArrayElement array = <helper_text_elements> element = { text = '\md = Löschen' }
    ...
    if IsXenon
        AddArrayElement array = <helper_text_elements> element = { text = '\m2 = Medium' }
        <helper_text_elements> = <array>
        RemoveParameter array
    endif
    if InFrontend
    ...
    if IsXenon
        ...
        if GetCardName
            FormatText textname = card_title 'Medium: %s' s = <card_name>
            CreateScreenElement {
                type = TextElement
                parent = files_menu
                font = testtitle
...
script files_menu_add_item pad_choose_script = NullScript font = dialog icon_alpha = 1.0
    ...
    if ((IsXbox)|| (isPC))
        if GotParam corrupt
            ...
            FormatText textname = filename 'BESCHÄDIGT %s' s = <filetype_name>
        endif
    endif
    CreateScreenElement {
        type = TextElement
    ...
    if GotParam total_file_size
        ...
        switch <platform>
            ...
            case xbox
            FormatText textname = file_size_text '%d Blöcke' d = <total_file_size>
            case ngc
            FormatText textname = file_size_text '%d Blöcke' d = <total_file_size>
        endswitch
    else
    ...
    if NOT ((GotParam badversion)|| (GotParam corrupt))
        switch <file_type>
            ...
            FormatText {
                ...
                '\c3Story: %a von %b Missionen / Klassik: %c von %d'
            ...
            FormatText {
                textname = files_menu_file_info_text
                '\c3ONLINE-EINSTELLUNGEN: %i'
                i = <network_id>
            }
            case cas
            <files_menu_file_info_text> = '\c3Mach-dir-den-Skater '
            case Park
            ...
            case createdgraphic
            <files_menu_file_info_text> = '\c3Mach-dir-die-Grafik '
        endswitch
    endif
    SetScreenElementProps {
        id = files_menu_file_info
    ...
    if GotParam index
        if GotParam save
            ...
            create_snazzy_dialog_box {
                ...
                buttons = [
                    {font = small text = 'Nein' pad_choose_script = launch_files_menu pad_choose_params = {filetype = <MenuFileType> save}}
                    {font = small text = 'Ja' pad_choose_script = delete_file pad_choose_params = <...> }
                ]
                destroy_on_event = 0
            }
...
script CreateNew 
    ...
    switch <file_type>
        ...
        <filename> = <name>
        case replay
        <filename> = 'Wiederholung'
        case CreatedGoals
        <filename> = 'Erstellte Ziele'
        case createdgraphic
        <filename> = 'Graphic'
        default 
        <filename> = 'Dooby Doo'
    ...
    create_onscreen_keyboard {
        ...
        keyboard_title = 'NAMEN SPEICHERN'
        text = <filename>
        min_length = 1
    ...
    if NOT CardIsFormatted
        if CardIsDamaged
            ...
            if IsPs2
                ...
                mem_card_message_pause NoTimerReset NoCardRemovalCheck
                goto mcmess_ErrorNotFormatted params = {QuitText = 'Ohne Formatieren fortfahren' backScript = NullScript}
            endif
        endif
    endif
    if (no_load = 0)
