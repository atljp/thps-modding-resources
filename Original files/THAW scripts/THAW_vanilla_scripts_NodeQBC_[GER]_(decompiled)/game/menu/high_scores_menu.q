...
script high_scores_menu_enter_initials 
    ...
    create_onscreen_keyboard {
        ...
        <...> 
        keyboard_title = 'INITIALEN EINGEBEN'
        min_length = 1
        max_length = 3
        heap = <heap>
    }
...
script high_scores_menu_create 
    ...
    if GotParam from_options
        ...
        create_helper_text {helper_text_elements = [{ text = '\b6/\b5 = Wählen' }
                { text = '\m1 = Zurück' }
            ]
        }
        CreateScreenElement {
        ...
        if GotParam from_pause
            ...
            create_helper_text helper_text_elements = [{ text = '\m0 = Fortfahren' }]
        endif
        <root_pos> = (100.0, 53.0)
        <top_bar_b> = (1.0, 36.0)
        <blue_pixel> = (1.0, 35.0)
...
script high_scores_change_level 
    ...
    if NOT GotParam current_index
        scriptassert 'Couldn\'t find current level name in level select table'
    endif
    begin 
    if GotParam RIGHT
