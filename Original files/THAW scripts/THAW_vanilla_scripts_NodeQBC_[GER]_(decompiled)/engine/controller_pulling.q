...
script create_controller_unplugged_dialog pad_choose_script = controller_refresh
    ...
    if GotParam leaving_net_game
        FormatText textname = text 'Controller wurde entfernt. Bitte schließe den Controller wieder an Controller-Anschluss %i an und drück die START-Taste, um zum Hauptmenü zu gelangen.' i = (<device_num> + 1)
        root_window ::settags leaving_net_game
    else
        if IsNGC
            FormatText textname = text 'Bitte den Controller in die Anschlussbuchse %i für Controller einstecken und START drücken, um fortzufahren.' i = (<device_num> + 1)
        else
            if IsPs2
                FormatText textname = text 'Controller an Controller-Anschluss %i anschließen und START-Taste drücken.' i = (<device_num> + 1)
            else
                if IsXenon
                    FormatText textname = text 'Drück START zum Fortfahren'
                else
                    FormatText textname = text 'Bitte schließe den Controller wieder an Controller-Anschluss %i an und drück die START-Taste, um fortzufahren.' i = (<device_num> + 1)
                endif
            endif
        endif
    endif
